...
#include "app/radio/RadioTetraAgent/src/RadioStatusSummary.h"
#include "app/radio/RadioTetraAgent/src/RadioConnectionManager.h"
...
void RadioConnectionManager::setControlConnection(EControlConnectionStatus controlConnectionStatus)
{
	...
	case ConnectToPrimary:
		...
		// We need to switch the active heartbeat monitoring here.
		if (m_active == false &&
			NULL != m_primaryConnection)
		{
			if (m_currentConnection != m_primaryConnection)
				m_currentConnection = m_primaryConnection;

			m_primaryConnection->setIgnoreIncomingData(false);
			m_primaryConnection->setCheckConnection(true);
		}

		//If it is possible that link between control agent and TCP server was broken, but the link between monitor agent and TCP server is health.
		//It will cause the session will not be released on TCP server. below connectionLost is used to protect it.
		if (m_active==false && m_secondaryConnection!=NULL)
		{
	...
	case ConnectToSecondary:
		...
		// We need to switch the active heartbeat monitoring here.
		if (m_active == false && 
			NULL != m_secondaryConnection)
		{
			if (m_currentConnection != m_secondaryConnection)
				m_currentConnection = m_secondaryConnection;
			
			m_secondaryConnection->setIgnoreIncomingData(false);
			m_secondaryConnection->setCheckConnection(true);
		}

		//If it is possible that link between control agent and TCP server was broken, but the link between monitor agent and TCP server is health.
		//It will cause the session will not be released on TCP server. below connectionLost is used to protect it.
		if (m_active==false && m_primaryConnection!=NULL)
		{
...
void RadioConnectionManager::onConnectionFail(bool isPrimary)
{
    ...
    if ( !isAbleToSwitch )
    {
		...
		if (m_active)  //only raise alarm on control mode
		{	
	...
	m_tcpServer.getRadio().setDpServerLostConnection(isPrimary ? 0 : 1, true);
	RadioStatusSummary::getInstance()->setConnectionStatus(m_isConnected);
	LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "[Connection Control] Current value: %d", m_controlConnectionStatus );	
	FUNCTION_EXIT;
}
...
void RadioConnectionManager::onConnectionResumed(bool isPrimary)
{
	...
	else if (/*m_active &&*/ m_currentConnection != 0 &&
		m_currentConnection->isPrimary() == isPrimary)
	{	// Connection resume we might have lost and reconnected so trigger method check connection again
		LOG0(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Restarting Method Check Connection due to momentary loss of connection because of hardware failover");
		m_currentConnection->setCheckConnection();
	}
	
	m_tcpServer.getRadio().setDpServerLostConnection(isPrimary ? 0 : 1, false);
	RadioStatusSummary::getInstance()->setConnectionStatus(m_isConnected);
	LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "[Connection Control] Current value: %d", m_controlConnectionStatus );
	FUNCTION_EXIT;
}
...
void RadioConnectionManager::setToControlMode(void)
{
	...
	// This solution is a work around, CRIS should manage the connection by themselves if and when 
	// ISCS tell them to switch the active connection.
	// Check the previous state if its from monitor
	bool bFromMonitor = false;
	if (m_operationMode == TA_Base_Core::Monitor)
	{
		LOG0(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Sleep for 3 seconds to allow CRIS to detect connection failure.");
		TA_Base_Core::Thread::sleep(3000); // Sleep 3 seconds to give CRIS radio server enough time to start the idle timer.
		bFromMonitor = true;
	}
	...
	if (m_primaryConnection != NULL)
    {
		...
		m_primaryConnection->setOperationMode(TA_Base_Core::Control);
		m_primaryConnection->setIgnoreIncomingData( m_primaryConnection != m_currentConnection );
		if (bFromMonitor && NULL != m_currentConnection)
			m_currentConnection->setCheckConnection(true);
    }
	...
	if (m_secondaryConnection != NULL)
    {
		...
		m_secondaryConnection->setOperationMode(TA_Base_Core::Control);
...
void RadioConnectionManager::setToMonitorMode(void)
{
	...
	LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugDebug,"Setting RadioConnectionManager to monitor mode with current connection = %d", m_controlConnectionStatus);
	m_active = false;
    ...
    {
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Start the sockets of primaryConnection processing events: setIgnoreIncomingData %d", m_primaryConnection != m_currentConnection);
		m_primaryConnection->setOperationMode(m_operationMode);
		m_primaryConnection->setIgnoreIncomingData(m_primaryConnection != m_currentConnection);
		...
		// We actually need this scenario so the existing call will not drop. We will follow the control agents current connection state.
		//m_primaryConnection->connectionLost(); 
    }
    ...
    {
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "Start the sockets of secondaryConnection processing events: setIgnoreIncomingData %d", m_secondaryConnection != m_currentConnection);
		m_secondaryConnection->setOperationMode(m_operationMode);
		m_secondaryConnection->setIgnoreIncomingData(m_secondaryConnection != m_currentConnection);
...
* @param    Method
*/
void RadioConnectionManager::SendMethod(const ITcpMethod& method, ta_uint32 transactionId)
{
    ...
    // ThreadGuard scope restriction
    {
        ...
        if (m_currentConnection != 0)
        {
            ...
            m_currentConnection->Send(method, transactionId);
        }
    }
