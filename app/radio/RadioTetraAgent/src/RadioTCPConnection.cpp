...
, m_operationMode(TA_Base_Core::NotApplicable)
{
    ...
    m_clientSocket->setReconnectInterval(2000); // Change to 2 seconds but allow to retry 3 times before declaring network lost

	m_clientSocket->setPingOption(false);

	m_RadioTimedEvent = new TA_IRS_App::RadioTimedEvent(&m_radioEventPocessor, true);

	unsigned long keepAliveInterval = 5;
	unsigned long keepAliveTimeOut = 50; 

	try
	{
		TA_Base_Core::RadioGlobalEntityDataPtr globalRadio(TA_Base_Core::RadioGlobalEntityData::getRadioGlobalsEntity());
		keepAliveInterval =  globalRadio->getKeepAliveInterval();
		keepAliveTimeOut = globalRadio->getKeepAliveTimeout();
	}
	...
	m_keepAliveProcessor = new RadioKeepAliveHandlerThread( this, keepAliveInterval, keepAliveTimeOut);
...
void RadioTCPConnection::setIgnoreIncomingData(bool bIgnore)
{
	...
	if ( m_ignoreIncomingData != bIgnore )
	{
		...
		// Update the Keep Alive Proccessor
		setCheckConnection();
	}
...
void RadioTCPConnection::setCheckConnection(bool bForceRestart /*= false*/)
{
	if ( !m_ignoreIncomingData )
	{
		m_keepAliveProcessor->startCheckConnection(bForceRestart);
	} 
	else
	{
...
* @param    method
*/
void RadioTCPConnection::Send(const ITcpMethod& method, ta_uint32 transactionId)
{
    FUNCTION_ENTRY("Send");
    ...
    const ta_uint32 dataSize = method.getMessageDataSize();
	...
	//	"#IMT# [resourceName:][resourceKeyWordId:][subsystem:TRAIN][direction:outgoing][transactionId:%lu][data:%s]",
	//	transactionId,
	//	hexText.c_str());
	...
	while (it!=data.end() && !m_terminited)
	{
		...
		if (m_event->isComplete())
		{
			...
			if ( m_keepAliveProcessor->processEvent( *event ) )
			{
				...
				else if (!m_ignoreIncomingData && m_operationMode == TA_Base_Core::Control)
			{
		...
		//RadioStatusSummary::getInstance()->setConnectionStatus(!isConnectionLost);
            }
        }
...
void RadioTCPConnection::resetConnection()
{
	FUNCTION_ENTRY("resetConnection");
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,
		"[Link Lost] Force Reset connection triggered. is primary link = %d", m_primary);
