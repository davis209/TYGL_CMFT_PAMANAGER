...
namespace TA_IRS_App
{
	...
	public:
		...
		*/
		 ta_uint32  getNewTransactionID(bool multipleEvents);

		
		ta_uint32 getNewTransactionIdNoWait ();

		/**
		* This method will cause the thread to block until the specified transaction event is received
		*/
		RadioEvent* waitForEvent(ta_uint32 TransactionID);
		...
		*/
		void waitListDeleteTransaction(ta_uint32 transactionID);
		...
		*/	
		void setWaitTimeout(ta_uint32 milliSecs);
	...
	private:
		...
		*/
		void waitListNewTransaction( ta_uint32  transactionID, bool multipleEvents);
		...
		*/
		bool waitListSignalTransaction(ta_uint32 transactionID,RadioEvent* event);
		...
		* Delete a transaction from the list
		*/
	//	void waitListDeleteTransaction(ta_uint32 transactionID); bihui 28/06/2011 change to public
		...
		*/
		TA_Base_Core::TimedWaitSemaphore* waitListGetSemaphore(ta_uint32 transactionID);

		ta_uint32 waitListGetElapsedTime(ta_uint32 transactionID);
		...
		*/
		RadioEvent* waitListGetEvent(ta_uint32 transactionID);

	private:

		typedef struct
		{
			ta_uint32                         transactionID;
			TA_Base_Core::TimedWaitSemaphore*	waitSemaphore;
			RadioEvent*							incomingEvent;
			std::queue<RadioEvent*>				incomingEvents;
			ta_uint32                         elapseMilliSecs;
			bool								sessionWaiting;
			bool								multipleEvents;
		} TransactionEntry;

		/**
		* The ta_int32 is the transactionID and the semaphore is used to signal the waiting process.
		*/
		std::vector<TransactionEntry> m_waitList;
		TA_Base_Core::ReEntrantThreadLockable   m_waitListGuard;

		ta_uint32               m_nextTransactionID;
		TA_Base_Core::ReEntrantThreadLockable   m_transactionCounterGuard;

		ta_uint32              m_waitTimeout;
