...
namespace TA_IRS_App
{
	...
	public:
		...
		*/
		...
		void setOcc2RadioSession(RadioSession& session);
		/**
		...
		*/
        ...
        RadioSession & getOcc2RadioSession();
		void ensureValidTrainSDSSession();
		...
		*/
        ...
        RadioEvent * sendMethod(RadioMethod& method, const RadioDirectorySynchronisation & radiodirsync);
		...
		*/
		...
		bool sendSDSTLMessage(const char* trainITSI, const TA_IRS_App::MessageListType& messages, std::string &status);
		bool sendUserDefinedSDSMessage(const char* trainITSI, const TA_IRS_App::MessageListType& messages, std::string &status);
		...
		void addQueuedBackgroundTask(RadioTaskPtr task);
		...
		*/
		...
		void setAllSubscribers(const TA_Base_Bus::IRadioCorbaDef::SubscriberList& list);
		void setAllGroups(const TA_Base_Bus::IRadioCorbaDef::GroupList& list);
		void setAllPatches(const TA_Base_Bus::IRadioCorbaDef::PatchList& list);
		void setAllSDS(const TA_Base_Bus::IRadioCorbaDef::PredefinedSDSMessageList& list);
		...
		*/
		...
		/************************************************************************/
		ta_uint32 getDefaultVolume() { return m_defaultVolume;};
		 ...
		 * getList
		 *
		 * Searches the selected calls database and retrieves any calls with the search text occuring in
		 * their name.
		 *
		 * @param mft the text by which to search the selected calls database
		 *
		 * @return The list of selected calls matching the parameter
		 */
		...
		virtual TA_Base_Bus::IRadioCorbaDef::CallStackHistoryList* getCSHListByType(const char* profile, ta_uint32 locationKey, TA_Base_Bus::IRadioCorbaDef::EStackCallType callType, bool readWrite);

		virtual void addCallStackEntry(const char* profile, ta_uint32 locationKey, ta_uint32 timestamp, TA_Base_Bus::IRadioCorbaDef::EOriginType originType, TA_Base_Bus::IRadioCorbaDef::EStackCallType callType, const char* identifier, const char* location, bool isEmergency, const char* tsi);
		virtual void removeCallStackEntry(ta_uint32 timestamp, const char* tsi);

		virtual void retrieveCallStackHistory();
		virtual void clearCallStackHistory();
		...
		virtual TA_Base_Bus::IRadioCorbaDef::PredefinedSDSMessage* getByShortName(const char* shortName);
		virtual TA_Base_Bus::IRadioCorbaDef::PredefinedSDSMessageList* getSDSList(bool readWrite);
		...
		virtual TA_Base_Bus::IRadioCorbaDef::RadioStatusSummary* getRadioStatusSummary();

		bool isActive(void);

		// Dynamic Group
		virtual void insertDBDynaGroup(const char* dgnaTSI, const char* dgnaName, const  std::vector<std::string>& destTSIs, const char * sessionId);
		virtual void insertDBDynaGroupMembers(IRASubscribers* group, const  std::vector<std::string>& destTSIs);
		virtual void deleteDBDynaGroup(const char* dgnaTSI);

		virtual void insertDBPatchGroup(const char* patchName, const char* ownerTSI, const  std::vector<std::string>& patchMemberTSIs);

		void addSubscriber(const SubscriberDetails& subscriber, const GroupList& groupList);
		void updateSubscriber(const SubscriberDetails& subscriber, const GroupList& groupList);
		void deleteSubscriber(const SubscriberDetails& subscriber, const GroupList& groupList);
    ...
    private:
		...
		*/
		...
		TA_Base_Core::IRASubscribers* _addNewSubscriber(const std::string& tsi,
		...
		void insertDBPatchGroupMembers(IRASubscribers* group, const  std::vector<std::string>& destTSIs);
       ...
       /// shared threadlock for the two session server console maps
		...
		TA_Base_Core::IRASubscribers::SubscriberType getSubscriberType(TA_Base_Bus::IRadioCorbaDef::SubscriberType type) const;
		...
		*/
        ...
        ///
		/// Radio session to use for Subscriber Directory Synchronisation.
        /// Having this in its own session isolates all the quirks of the
        /// K_SEARCH_MODE=FULL SearchSubscriber functionality that otherwise
        /// affect emergency regroup, train TRN call forwarding and others.
		RadioSession                          * m_Occ2RadioSession;
		...
		// Added to process the SubscriberActivityEvent and GroupCall Incoming event Group Call Event
		TA_Base_Core::WorkItemQueueProcessor*	m_backgroudQueueProcessor; // Queued
        ...
        ///
        /// flag to indicate control/monitor mode
		bool									m_active;
		...
		ta_uint32                             m_defaultVolume;
		RadioServerLinkStatusUpdateThread*		m_serverLinkStatusThread;
	};
    inline bool Radio::isActive(void)
    {
		//FUNCTION_ENTRY("getIsActive");
		//FUNCTION_EXIT;
		return m_active;
    }
	
    inline RadioSession & Radio::getISCSRadioSession()
    {
		ensureValidTrainSDSSession();
        return *m_ISCSRadioSession;
    }

    inline RadioSession & Radio::getOcc2RadioSession()
    {
		ensureValidTrainSDSSession();
        // use OCC2 if available
        if (m_Occ2RadioSession!=NULL)
        {
            return *m_Occ2RadioSession;
        }

        return *m_ISCSRadioSession;
    }

	inline void Radio::ensureValidTrainSDSSession()
	{
		if (m_ISCSRadioSession == NULL)
		{	// Throw exception
			LOG0(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Radio Session configuration exception!: Train SDS Session is Disabled");
			TA_THROW(RadioSessionException("Train SDS Session is Disabled", 0));
		}
	}
};
