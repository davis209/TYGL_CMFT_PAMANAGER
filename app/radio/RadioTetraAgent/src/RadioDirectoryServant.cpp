...
ta_uint32 RadioDirectoryServant::m_locationKey = 0; // Set to 0 initially
...
ta_int32 RadioDirectoryServant::getLastDirectorySynchronisationTime()
{
	TA_Base_Bus::GenericAgent::ensureControlMode();
    ta_int32 last_synch_time = 0;
...
void RadioDirectoryServant::onSubscriberChange(TA_Base_Core::IRASubscribers * subscriber, SubscriberChangeType changeType)
{
	...
	notifySubscriberChange(subscriber, changeType);
...
void RadioDirectoryServant::notifySubscriberChange(TA_Base_Core::IRASubscribers * subscriber, SubscriberChangeType changeType)
{
	...
	if (subscriber != 0)
	{
		...
		default:
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "Invalid subscriber type, taking shortcut out of function");
			return;
			break;
		}
	}
	...
	TA_Base_Core::CommsMessageSender* commsMessageSender =
		TA_Base_Core::MessagePublicationManager::getInstance().getCommsMessageSender(TA_Base_Core::RadioComms::RadioDirectoryNotification, m_locationKey, m_locationKey, NsaTypeRegular);
