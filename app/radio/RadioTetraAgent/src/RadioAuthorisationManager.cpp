...
*/
bool RadioAuthorisationManager::cancelRequest(CADReference callID)
{
	...
	{  // nest for lock scope
		...
		T_AuthRequestMap::iterator itFind = m_radioAuthorisationStack.find(callID);
		if (itFind != m_radioAuthorisationStack.end())
		{
			request = itFind->second;
			m_radioAuthorisationStack.erase(itFind);
			//Broadcast
...
void RadioAuthorisationManager::getAuthorisationList(AuthorisationStack& authorisationList, SessionReference SessionRef) //TD19479
{
	...
	T_AuthRequestMap::iterator it = m_radioAuthorisationStack.begin();
	for (it ; it != m_radioAuthorisationStack.end(); it ++ )
	{
		if (it->second.sessionRef == SessionRef)
		{
			authorisationList.push_back(it->second);
		}
	}
	FUNCTION_EXIT;
}
...
void RadioAuthorisationManager::authoriseCall(CADReference callID,bool accept,const char* sessionID)
{
	...
	{  // nest for lock scope
		TA_Base_Core::ThreadGuard  guard(m_authorisationStackLock);
		m_radioAuthorisationStack.erase(callID);	
...
void RadioAuthorisationManager::getCADDetails(SessionReference sessionRef,CADReference cadRef)
{
    ...
    if ( num_details == 2 )
    {
        request.callerName      = subscriberDetails[(ta_uint32 )RadioMethodQueryReference::K_NAME];
        request.callerLocation  = subscriberDetails[(ta_uint32 )RadioMethodQueryReference::K_ORGANISATION];
    ...
    if ( num_details == 2 )
    {
        request.calledName      = subscriberDetails[(ta_uint32 )RadioMethodQueryReference::K_NAME];
        request.calledLocation  = subscriberDetails[(ta_uint32 )RadioMethodQueryReference::K_ORGANISATION];
	...
	// Add to the list
	TA_Base_Core::ThreadGuard  guard(m_authorisationStackLock);
	m_radioAuthorisationStack.insert(T_AuthRequestMap::value_type(cadRef, request));
 ...
 */
std::string RadioAuthorisationManager::queryResults(ta_uint32 sessionRef,
													ta_uint32 resultRef,
...
void RadioAuthorisationManager::applyFullState(const RadioAuthorisationManagerType& state)
{
	...
	
    {
        ...
        {
			m_radioAuthorisationStack.insert(T_AuthRequestMap::value_type(it->callID, *it));
        }
...
void RadioAuthorisationManager::getFullState(RadioAuthorisationManagerType& state)
{
	...
	T_AuthRequestMap::const_iterator it;
	
	{
		...
		{
			state.radioAuthorisationStack.push_back(it->second);
		}
...
void RadioAuthorisationManager::updateAuthorisationRequest(const AuthorisationRequestType& request)
{
	...
	m_radioAuthorisationStack[request.callID] = request;
...
void RadioAuthorisationManager::removeAuthorisationRequest(CADReference callID)
{
	...
	TA_Base_Core::ThreadGuard  guard(m_authorisationStackLock);
	T_AuthRequestMap::iterator itrFind = m_radioAuthorisationStack.find(callID);
	if (itrFind != m_radioAuthorisationStack.end())
	{
		m_radioAuthorisationStack.erase(itrFind);
	}
...
void RadioAuthorisationManager::sessionLoggedOut(SessionReference sessionRef)
{
    ...
    { // ThreadGuard scope
        ...
        TA_Base_Core::ThreadGuard  guard(m_authorisationStackLock);
		T_AuthRequestMap::iterator it=m_radioAuthorisationStack.begin();
		while (it!=m_radioAuthorisationStack.end())
		{
			if (it->second.sessionRef == sessionRef)
			{
				LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, 
					"Erasing authorisation request %lu for session %d", it->second.callID, sessionRef);
