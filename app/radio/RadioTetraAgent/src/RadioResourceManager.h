...
namespace TA_IRS_App
{
    ...
    typedef enum AudioSpeaker { LeftSpeaker, RightSpeaker };
	...
	class RadioResourceManager : TA_Base_Core::Thread
	{
		...
		public:
			...
			*/
			void setAudioSpeaker(SessionReference SessionRef, CallReference callId, AudioSpeaker audioSpeaker, int volumeDelta, RadioMethodChangeReference::MicrophoneState micState);
            ...
            */
             ...
             */
			ta_int32 releaseAudioResources(SessionReference sessRef, CallReference callRef);
		...
		private:
			...
			/*
			* This function checks if an audio resouse has already been assigned to this call reference
			*
			* @param   SessionRef    indicates the owner session
			* @param   CallRef       call reference to be assoicated with the audio reference
			*/
			bool isAudioAttached(SessionReference sessionRef, CallReference callRef);
		private:
			...
			*/
            ...
            ///
            /// Map of AudioEntry structs, keyd by AudioReference
			typedef std::map<AudioReference, AudioEntry >        AudioEntryMap;
			
            ///
            /// Map of AudioEntryMap pointers, keyd by SessionReference
			typedef std::map<SessionReference, AudioEntryMap* >  SessionAudioMap;

            ///
            /// The map of sessions' audio resources
			SessionAudioMap			m_sessionAudioMap;

            ///
            /// ThreadLock for the m_sessionAudioMap
			TA_Base_Core::ReEntrantThreadLockable   m_audioMapLock;

            ///
            /// volume that call was last set to
			ta_int32 m_currentAudioVolume;
			//TD10963 marc_bugfix the default volume 
			ta_uint32      m_DefaultVolume;
			std::map<SessionReference, ta_int32>     m_sessionVolume;
            ...
            ///////////////////////////////
            // Encapsulating SessionAudioMap/AudioEntryMap access
            ///////////////////////////////

            /**
             * Retrieves AudioEntry struct for given session/call
             * @param sessionRef
             * @param callRef
             * @param audioRef
             * @param audioEntry - set to the audio entry for the call. This can later be used in 
             * @return true if the AudioEntry was found (it should have been)
             */ 
            bool findAudioEntry(SessionReference sessionRef, CallReference callRef, AudioReference & audioRef, AudioEntry & audioEntry);

            bool useAudioEntry(SessionReference sessionRef, CallReference callRef, AudioReference & audioRefs);
            ta_int32 useAudioEntries(SessionReference sessionRef, CallReference callRef, std::vector<AudioReference> & audioRefs, bool isMonitor=true, ta_int32 numEntriesRequired=1);

            void freeAudioEntry(SessionReference sessionRef, AudioReference audioRef);
            void freeAudioEntries(SessionReference sessionRef, std::vector<AudioReference> & audioRefs);

            void storeAudioEntry(SessionReference sessionRef, AudioReference audioRef, const AudioEntry & audioEntry);
