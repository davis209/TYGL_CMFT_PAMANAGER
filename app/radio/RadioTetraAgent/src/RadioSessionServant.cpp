/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/TYGL_CMFT_TIP/J155/transactive/app/radio/RadioTetraAgent/src/RadioSessionServant.cpp $
  * @author:  Andy Parker
  * @version: $Revision: #5 $
  *
  * Last modification: $DateTime: 2026/03/16 16:45:10 $
					  ...
					  m_transactiveSession("")
{
    ...
    m_flexibleTimeout = 300;
    try
    {
        // radio timeout x 2 to be safe.
        m_flexibleTimeout = globalEntityData.getRadioTransactionTimeout() * 2;
		if (m_flexibleTimeout > 300)
		{
			LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugWarn, 
				"The flexibleTimeout configuation of RadioServant is too big(%d). Set it to maximum(300)", 
				m_flexibleTimeout);
			m_flexibleTimeout = 300;
		}
    }
	...
	m_radioAgentEntityKey = entity->getKey();
	m_radioAgentLocationKey = entity->getLocation();

	ta_uint32 audioLink = entityData->getAudioLinkId();
	ta_uint32 audioLinkServerId = entityData->getAudioLinkServerId();
	std::string ipAddressGWLD =  entityData->getIPAddressGWLD();
	ta_uint32 switchUserWaitPeriod = entityData->getSwitchUserWaitPeriod();

    LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
           "Constructing RadioSessionServant %s (%s), audioLink %u, audioLinkServer %u, IPAddressGWLD = %s",
            m_entityName.c_str(), entityData->getDescription().c_str(), audioLink, audioLinkServerId, ipAddressGWLD.c_str()); 
    RadioTcpServer * tcpServer = radio.getAudioLinkServer(audioLinkServerId);
    if (0==tcpServer)
    {
        TA_THROW(RadioConfigurationException("AudioLinkServerId out of range"));
    }

    bool isISCSSession = entityData->useForTrainCommunication();
    bool isOcc2Session = ((0 == m_entityName.compare(RadioAgentConstants::OCC2_RADIO_SESSION_NAME)) || 
		(0 == m_entityName.compare(RadioAgentConstants::BOCC2_RADIO_SESSION_NAME)));

    std::string autoLoginName("");

    // these are the auto login sessions
    if ( isISCSSession || isOcc2Session)
    {
		LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
			"RadioSessionServant %s | isISCSSession = %d |isOcc2Session = %d ",
			m_entityName.c_str(), isISCSSession, isOcc2Session);
        ...
        if ( autoLoginName.empty() )
        {
            ...
            else if( isOcc2Session )
            {
                // only at occ
                autoLoginName = RadioAgentConstants::DEFAULT_OCC2_LOGIN_NAME;
            }
        }
    ...
    if (isOcc2Session)
    {
        radio.setOcc2RadioSession(*m_radioSession);
    }
    ...
    //{
	// comment off the sessionUpdate as well as the eventSubscriber
    ...
    m_commsMessageSender = TA_Base_Core::MessagePublicationManager::getInstance().getCommsMessageSender( 
		TA_Base_Core::RadioComms::Context, m_radioAgentLocationKey, m_radioAgentLocationKey, NsaTypeRegular);
...
void RadioSessionServant::receiveSpecialisedMessage(const TA_Base_Core::CommsMessageCorbaDef& message)
{
	...
	//	}
	//}
	/*else*/ if (0 == mesgTypeKey.compare(TA_Base_Core::RadioComms::RadioRetrieveCallStackHistory.getTypeKey()))
	{
		TA_Base_Bus::IRadioCorbaDef::CallStackHistoryMessage* update = 0;

		if ((message.messageState >>= update) != 0)
		{

			// Check the message content
			if (update->toRetrieve)
			{
				LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Retrieve Call Stack History Data");
				m_radio.retrieveCallStackHistory();
...
void RadioSessionServant::receiveCallFwdRequest(const char* sessionID)
{
	FUNCTION_ENTRY("receiveCallFwdRequest");

	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "receiveCallFwdRequest");
    ...
    //    auto active_sessions = m_authenticationLibrary.getActiveSessions(m_entityLocation);
	//
    ...
    //        {
				...
				m_radioSession->setCallForwarding(cfwdDetails, sessionID);
...
void RadioSessionServant::sessionLogin(std::string sessionId)
{
 ...
 //   //std::cout << "Getting Session Information" << std::endl;
	////LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo,
	////	"Getting radio session Information");
 ...
 //   delete profile;
	//
	//if ( true == loginName.empty() /*|| strOperator.find("AgentOperator") != -1*/)
	//{		
 ...
 //   sessionLogin(loginName, password, sessionId, profileName);

    FUNCTION_EXIT;
}

void RadioSessionServant::sessionLoginViaAgent(const char* loginName, const char* password, const char* sessionId)
{
	FUNCTION_ENTRY("sessionLoginViaAgent(loginName,password,sessionId,sesProfileName)");

	TA_Base_Bus::GenericAgent::ensureControlMode();
	LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo,
		"[sessionLoginViaAgent] Login via Agent radio session %s (session id: %s, login name: %s, password: %s)", m_entityName.c_str(), sessionId, loginName, password);
...
void RadioSessionServant::sessionLogin(const std::string& loginName, const std::string& password, const std::string& sessionId, const std::string& sesProfileName)
{
    FUNCTION_ENTRY("sessionLogin(loginName,password,sessionId)");
	TA_Base_Core::ThreadGuard guard(m_loginLock);
	m_transactiveSession = sessionId;
	//m_radioSession->setSessionProfileName(sesProfileName);
	...
	//ta_uint32 unOccLocation = TA_Base_Core::LocationAccessFactory::getInstance().getOccLocationKey();
	//ta_uint32 unDepotLocation = TA_Base_Core::LocationAccessFactory::getInstance().getDepotLocationKey();
...
ta_int32 RadioSessionServant::getSessionReference(const char * sessionId)
{
...
char* RadioSessionServant::getSessionTsi(const char * sessionId)
{
	...
	{
		// Get the session tsi
		tsi = m_radioSession->getSessionTSI();
		LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, 
			"getSessionTsi returned %s",tsi.c_str());
	}
	catch (RadioSessionException& e)
	{
		// Error excuting the command
	...
	FUNCTION_EXIT;
	return CORBA::string_dup(tsi.c_str());
}

void RadioSessionServant::answerCall(ta_int32 callID,const char * sessionId)
{
...
ta_int32 RadioSessionServant::makeCall(const char * destinationTSI, TA_Base_Bus::IRadioSessionCorbaDef::ECallType calltype, bool isFullDuplex,TA_Base_Bus::IRadioSessionCorbaDef::ECallPriority priority, bool hook, const char * sessionId, bool isInvisibleToGUIApps, ta_int32 locationKey)// //TD19783
{
...
void RadioSessionServant::releaseCall(ta_int32 callID,const char * sessionId)
{
...
void RadioSessionServant::holdCall(ta_int32 callId,const char * sessionId)
{
...
void RadioSessionServant::resumeCall(ta_int32 callId,const char * sessionId)
{
...
ta_int32 RadioSessionServant::startMonitoring(const char * tsi, const char * sessionID)
{
	...
	try
	{
		retVal = m_radioSession->startGroupMonitoring(tsi);
	}
	catch (RadioException& e)
	{
		// Error excuting the command
...
void RadioSessionServant::stopMonitoring(const char * tsi, const char * sessionID)
{
	...
	try
	{
        m_radioSession->stopGroupMonitoring(tsi);
	}
	catch (RadioException& e)
	{
...
ta_int32 RadioSessionServant::oneShotMonitoring(const char* TSI, const char* sessionID)
{
	...
	LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugDebug,
		"oneShotMonitoring() - tsi=%s sessionID=%s", TSI, sessionID);
	ta_int32 retVal = 0;
	try
	{
		retVal = m_radioSession->oneShotMonitoring(TSI);
	}
	catch (RadioException& e)
	{
		// Error executing the command
...
//// no longer in use, use getMonitoredGroup instead
TA_Base_Bus::IRadioSessionCorbaDef::MonitoredSubscriberList* RadioSessionServant::getMonitoredSubscribers()
{
	FUNCTION_ENTRY("getMonitoredGroups");
	//TA_Base_Bus::GenericAgent::ensureControlMode();

	//SessionReference sessionRef = m_radioSession->getValidSessionRef();

	//LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugDebug,
	//	"getMonitoredSubscribers() - sessionRef=%d", m_radioSession->getValidSessionRef());

	//TA_Base_Bus::IRadioSessionCorbaDef::MonitoredSubscriberList* pRetVal = m_radioSession->getMonitoredSubscribers(sessionRef);
	TA_Base_Bus::IRadioSessionCorbaDef::MonitoredSubscriberList* pRetVal;
	FUNCTION_EXIT;
	return pRetVal;
}

TA_Base_Bus::IRadioSessionCorbaDef::MonitoredGroupList* RadioSessionServant::getMonitoredGroup()
{
	FUNCTION_ENTRY("getMonitoredGroups");
	TA_Base_Bus::GenericAgent::ensureControlMode();
	...
	TA_Base_Bus::IRadioSessionCorbaDef::MonitoredGroupList* pRetVal = m_radioSession->getMonitoredGroup(sessionRef);

	LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugDebug,
		"getMonitoredGroups() - length=%d", pRetVal->length());
...
ta_int32 RadioSessionServant::startListening(ta_int32 callId,const char * sessionId)
{
...
void RadioSessionServant::stopListening(ta_int32 callId, const char * sessionId)
{
...
ta_int32 RadioSessionServant::joinCall(ta_int32 callId, const char * sessionId)
{
...
void RadioSessionServant::selectCall(ta_int32 callId,const char * sessionId)
{
...
void RadioSessionServant::deselectCall(ta_int32 callId,const char * sessionId)
{
...
void RadioSessionServant::adjustCallVolume(ta_int32 callId,bool increase,const char * sessionId)
{
...
char* RadioSessionServant::makeDynamicGroup(const char* dgnaTSI, const char* dgnaName, const TA_Base_Bus::IRadioSessionCorbaDef::DestinationTSIList& destTSIs, const char * sessionId)
{
    ...
    try
    {
		strRetVal = m_radioSession->makeDynamicGroup(destlist,sessionId);
    ...
    FUNCTION_EXIT;
    return CORBA::string_dup(strRetVal.c_str());

}

void RadioSessionServant::updateDynamicGroup(const char* dgnaTSI, const char* dgnaName, const ::TA_Base_Bus::IRadioSessionCorbaDef::DestinationTSIList& destTSIs, const char* sessionID)
{
	...
	try
	{
		m_radioSession->updateDynamicGroup(destlist, sessionID);
	}
	catch (RadioSessionException& e)
	{
		// Error excuting the command
...
void RadioSessionServant::deleteDynamicGroup(const char* dgnaTSI, const char* sessionID)
{
	FUNCTION_ENTRY("deleteDynamicGroup");

	LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo,
		"Deleting dynamic group with tsi %s", dgnaTSI);

	try
	{
		m_radioSession->deleteDynamicGroup(dgnaTSI, sessionID);
...
* @param    sessionId
*/
CORBA::Boolean RadioSessionServant::sendTextMessage(const char* destinationTSI, 
                               const TA_Base_Bus::IRadioSessionCorbaDef::MessageListType& messageList,
                               ...
                               CORBA::String_out status)
{
    ...
    LOG( SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugInfo, 
         "Sending text message to '%s'",destinationTSI);
    ...
    for (unsigned int messageIndex = 0; messageIndex < numMessages; ++messageIndex)
	{
		...
		{
            unsigned char nextChar = messageList[messageIndex][textIndex];
			messageText.push_back( nextChar );
            auditText.append( 1, nextChar );
		}
    ...
    try 
    {
        ...
        if (m_radioSession->sendTextMessage(destinationTSI, messages, Unknown, sessionId, strStatus, true))
        {
            status = CORBA::string_dup(strStatus.c_str());
            bStatus = true;
        }
    }
    catch (RadioSessionException& e)
	...
	//TA_Base_Core::NameValuePair messagePair("TextMessage", auditText);
	//parameters.push_back(&destPair);
...
CORBA::Boolean RadioSessionServant::sendUserDefinedSDS(const char* destinationTSI, const TA_Base_Bus::IRadioSessionCorbaDef::MessageListType& messageList, const char* sessionId, CORBA::String_out status)
{
	...
	{
		std::string strStatus;
		if (m_radioSession->sendUserDefinedSDS(destinationTSI, messages, sessionId, strStatus, true))
		{
			status = CORBA::string_dup(strStatus.c_str());
			bStatus = true;
		}
...
void RadioSessionServant::terminateCall(ta_int32 callId, const char * sessionId, bool bIsMonitored)
{
...
ta_uint32 RadioSessionServant::getEntityKey()
{
	return this->m_entityKey;
}

ta_uint32 RadioSessionServant::getEntityLocationKey()
{
...
void RadioSessionServant::checkPermission(std::vector<TA_Base_Bus::AccessControlledAction> actions, std::string session)
{
 ...
 //   ta_uint32 resourceID = resource->getKey();
 //   delete resource;
...
void RadioSessionServant::onSessionInitialisationComplete()
{
	...
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "onSessionInitialisationComplete, Entity:%s", m_entityName.c_str());

	std::vector<TA_Base_Core::IEntityData*> entityList;
	TA_Base_Core::IEntityData*  entity;
	std::string   radioEntityName;
	ta_uint32 locationKey = atol(RunParams::getInstance().get(RPARAM_LOCATIONKEY).c_str());
	entityList = TA_Base_Core::EntityAccessFactory::getInstance().getEntitiesOfTypeAtLocation(TA_Base_Core::RadioAgentEntityData::getStaticType(), locationKey);
	...
	m_radioAgent.setEntityName(radioEntityName, false);
	CORBA_CALL(m_radioAgent, sessionInitialisation, (locationKey, m_consoleID));
    ...
    //                    {
    //                        std::string loginName = profile->getItsiGroupName();
				//			std::string sesProfileName = profile->getName();
    //                        delete profile;
    //                        
    //                        // TODO - modify the SE_PROFILE_V and the associated DAI to retrieve password
    //                        std::string password;
    ...
    //                        if ( !loginName.empty() )
    //                        {
    //                            sessionLogin(loginName, password, sess_info.SessionId, sesProfileName);
				//				// We need to set the current session of the event subscriber in order to handle
				//				// the session end properly.
				//				//m_eventSubscriber->setCurrentSessionID(sess_info.SessionId); // Set the session ID
...
::CORBA::Boolean RadioSessionServant::getRadioSessionIsLogin()
{
	FUNCTION_ENTRY("getRadioSessionIsLogin");	
	FUNCTION_EXIT;
	return m_radioSession->isLoggedIn();
}
...
void RadioSessionServant::setToControlMode()
{
   ...
   //         NULL,
	...
	MessageSubscriptionManager::getInstance().subscribeToCommsMessage(
		TA_Base_Core::RadioComms::RadioRetrieveCallStackHistory,
		this,
		0,
		0,
		atol(loc.c_str()),
...
unsigned short RadioSessionServant::getFlexibleTimeOutValue(void)
{
...
std::string RadioSessionServant::getCurrentSession()
{
	FUNCTION_ENTRY("getCurrentSession");
	FUNCTION_EXIT;
	return m_transactiveSession;
}
...
void RadioSessionServant::authoriseCall(ta_int32 callID,bool accept,const char* sessionID)
{
...
ta_int32 RadioSessionServant::includeSubscriber(ta_int32 callId,const char * subscriber,const char * sessionId)
{
	...
	try
	{
		new_callId = m_radioSession->includeSubscriber(callId,subscriber);
	}
	catch (RadioSessionException& e)
	{
		// Error excuting the command
