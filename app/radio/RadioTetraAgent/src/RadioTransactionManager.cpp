...
*/
 ta_uint32  RadioTransactionManager::getNewTransactionID(bool multipleEvents)
{
	...
	waitListNewTransaction(m_nextTransactionID, multipleEvents);

	FUNCTION_EXIT;
	return m_nextTransactionID;
}

RadioEvent* RadioTransactionManager::waitForEvent(ta_uint32 transactionID)
{
	...
	ta_uint32 timeElapsed = waitListGetElapsedTime(transactionID);
...
void RadioTransactionManager::setWaitTimeout(ta_uint32 milliSecs)
{
...
void RadioTransactionManager::waitListNewTransaction( ta_uint32  transactionID, bool multipleEvents)
{
	...
	entry.multipleEvents = multipleEvents;
...
bool RadioTransactionManager::waitListSignalTransaction(ta_uint32 transactionID,RadioEvent* event)
{
	...
	do
	{
		...
		for (it=m_waitList.begin();it!=m_waitList.end(); it++)
		{
			...
			{
				if (it->multipleEvents)
				{
					it->incomingEvents.push(event);
					it->waitSemaphore->post();
					FUNCTION_EXIT;
					return true;
				}
...
TA_Base_Core::TimedWaitSemaphore* RadioTransactionManager::waitListGetSemaphore(ta_uint32 transactionID)
{
...
ta_uint32 RadioTransactionManager::waitListGetElapsedTime(ta_uint32 transactionID)
{
...
RadioEvent* RadioTransactionManager::waitListGetEvent(ta_uint32 transactionID)
{
	...
	for (it=m_waitList.begin();it!=m_waitList.end(); it++)
	{
		if (it->transactionID==transactionID)
		{
			if (it->multipleEvents)
			{
				if (it->incomingEvents.size() != 0)
				{
					event = it->incomingEvents.front();
					it->incomingEvents.pop(); // remove the top event
					FUNCTION_EXIT;
					return event;
				}
			}
...
void RadioTransactionManager::waitListDeleteTransaction(ta_uint32 transactionID)
{
...
ta_uint32 RadioTransactionManager::getNewTransactionIdNoWait ()
{
