...
namespace TA_IRS_App
{
					 ...
					 : m_event(event)
	{
		// Check the type and size matches the expected event!
			if (parameterLength!=event.getParameterLength()) 
			{
				TA_THROW(RadioProtocolException("Reader parameter length does not match event parameter length."));
			}
...
ta_uint32 RadioEventReader::getParameterLong(unsigned int position) const
	{
	...
	ta_uint32 RadioEventReader::ENCODE_REFERENCE(ta_uint32 r ) const
	{
		if ( 10000 < r && r < 100000 )
		{
			r = ( r * 100 ) + getEvent().getSessionRef();
		}
		return r;
	}
	// ++limin CALL REFERENCE
