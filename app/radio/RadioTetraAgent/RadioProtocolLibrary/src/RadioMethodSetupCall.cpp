...
void RadioMethodSetupCall::setCallType(ECallType callType)
{
    ...
    // clear the Ambient flag and lowest three bits
    m_callType &= 0x7FF8;
	...
	else if (callType==AMBIENT_LISTENING)
	{
		m_callType |= 0x8000;
        setFullDuplex(false);
	}
...
void RadioMethodSetupCall::setEncriptionFlag(bool on/*=true*/)
{
	FUNCTION_ENTRY("setEncriptionFlag");
...
void RadioMethodSetupCall::setSlotPerFrame(SLotPerFrame sf)
{
	FUNCTION_ENTRY("setSlotPerFrame");
	m_callType &= 0xF9FF; // Clear the flag first
	if (sf == One_Slot)
	{ // do nothing
	}
	else if (sf == Two_Slots)
	{
		m_callType |= 0x0200;
	}
	else if (sf == Three_Slots)
	{
		m_callType |= 0x0400;
	}
	else if (sf == Four_slots)
	{
		m_callType |= 0x0600;
	}
	setParameterShort(SETUP_CALL_TYPE_START,m_callType);
...
void RadioMethodSetupCall::setRequestTx(ERequestTx rqstx)
{
    ...
    setParameterLong(SETUP_CALL_REQUESTTX_START,(ta_uint32 )rqstx);
    FUNCTION_EXIT;
}
