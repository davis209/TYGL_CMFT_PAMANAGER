 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/GuiHelper.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #4 $
 *
...
using TA_Base_Core::DataConversion;

namespace TA_Base_Core
{

    const ta_uint32 GuiHelper::MAX_INSTANCES_LIMIT = 5;

    GuiHelper::GuiHelper(const ta_uint32 key):
        m_isValidData(false),
        m_isNew(false),
        m_key(key),
        m_dateCreated(0),
    ...
    GuiHelper::GuiHelper(const ta_uint32 row, TA_Base_Core::IData& data)
        :   m_isNew(false),
            m_isValidData(false)
    {
        FUNCTION_ENTRY("GuiHelper(const ta_uint32 , TA_Base_Core::IData&)");
    ...
    GuiHelper::GuiHelper(const GuiHelper& theGuiHelper)
        : m_isValidData(false),
          ...
          m_maxInstances(theGuiHelper.m_maxInstances),
    ...
    ta_uint32 GuiHelper::getAppMaxInstances()
    {
    ...
    void GuiHelper::setAppMaxInstances(const std::string& numInstances)
    {
        FUNCTION_ENTRY("setAppMaxInstances");
        ...
        m_maxInstances = DataConversion::toUnsignedInteger(numInstances);
    }
    ...
    {
        FUNCTION_ENTRY("GuiHelper::invalidate");
        // This method cannot be called until the key has been set
    ...
    {
        FUNCTION_ENTRY("GuiHelper::reload");
    ...
    void GuiHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data)
    {
        ...
        m_maxInstances = data.getUnsignedLongData(row, columnNames[5]);
    ...
    void GuiHelper::deleteThisGui()
    {
        ...
        // get a connection to the database
        IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase(OnlineUpdatable_Cd/*OnlineUpdatable_Cd*/, Write);
    ...
    void GuiHelper::addNew()
    {
        ...
        LOG_DEBUG("Gui %s is a new gui. It will be added to the database.", m_name);
        ...
        if (m_maxInstances > MAX_INSTANCES_LIMIT)
        {
            delete data;
            data = NULL;

            TA_THROW(DataException("Max instances is invalid.", DataException::INVALID_VALUE, "max instances"));
        }

        ta_uint32 key = data->getUnsignedLongData(0, "key");
        delete data;
        data = NULL;
        ...
        databaseConnection->prepareSQLStatement(strSql, GUIAPPLICATION_INSERT_16801, key, databaseConnection->escapeInsertString(m_name),
                                                ...
                                                m_maxInstances, m_shouldReposition, m_shouldPassParameters, m_isIconDisplayed, m_shouldTerminateOnLogout);
    ...
    void GuiHelper::modifyExisting()
    {
        ...
        LOG_DEBUG("Gui %s already exists. It's details will be updated.", m_name);
        if (m_maxInstances > MAX_INSTANCES_LIMIT)
        {
            TA_THROW(DataException("Max instances is invalid.", DataException::INVALID_VALUE, "Max Instances"));
        }
        ...
        databaseConnection->prepareSQLStatement(strSql, GUIAPPLICATION_UPDATE_16651,
                                                ...
                                                databaseConnection->escapeQueryString(m_helpFilePath), m_maxInstances, m_shouldReposition, m_shouldPassParameters, m_isIconDisplayed,
