 ...
 * in any form.
 *
 * Source: $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/LocationHelper.cpp $
 * @author Karen Graham
 * @version $Revision: #4 $
...
namespace TA_Base_Core
{
    ...
    LocationHelper::LocationHelper(const ta_uint32 key)
        : m_key(key),
          m_name(""),
          m_description(""),
          m_orderId(0),
    ...
    LocationHelper::LocationHelper(const ta_uint32 key, const std::string& name, const std::string& description, const ta_uint32 orderId,
    ...
    LocationHelper::LocationHelper(const ta_uint32 row, TA_Base_Core::IData& data)
        :
        m_isValidData(false),
        m_isValidProfileData(false),
        m_isNew(false)
    {
        FUNCTION_ENTRY("LocationHelper(const ta_uint32 , TA_Base_Core::IData&)");
    ...
    ta_uint32 LocationHelper::getKey()
    {
    ...
    ta_uint32 LocationHelper::getOrderId()
    {
        // We don't want to call reload() if this is a new location and it has not been written to the database
        if (!m_isValidData && !m_isNew)
        {
    ...
    void LocationHelper::setOrderId(const ta_uint32 orderId)
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    std::string LocationHelper::getDefaultDisplay(ta_uint32 profileKey, int displayNumber)
    {
    ...
    bool LocationHelper::isProfileAssociated(ta_uint32 profileId)
    {
        ...
        std::vector<ta_uint32>::iterator result = std::find(m_associatedProfileKeys.begin(),
    ...
    void LocationHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data)
    {
    ...
    void LocationHelper::deleteThisLocation()
    {
        ...
        catch (const DatabaseException& ex)
        {
            ...
            if (errorMsg.find(KEY_WORD) != std::string::npos)
            {
                ...
                TA_THROW(DataException(message.c_str(), DataException::CANNOT_DELETE, ""));
            }
        }
    ...
    void LocationHelper::addNewLocation()
    {
        ...
        LOG_DEBUG("Location %s is a new location. It will be added to the database.", m_name);
		...
		databaseConnection->prepareSQLStatement(strSql, LOCATION_INSERT_7351,
                                                databaseConnection->getNextPkey("LOCATION_SEQ"), 
    ...
    void LocationHelper::modifyExistingLocation()
    {
        ...
        LOG_DEBUG("Location %s already exists. It's details will be updated.", m_name);
		...
		databaseConnection->prepareSQLStatement(strSql, LOCATION_UPDATE_7151, 
