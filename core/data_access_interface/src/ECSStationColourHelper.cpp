 ...
 * in any form.
 *
 * Source: $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/ECSStationColourHelper.cpp $
 * @author Andy Siow
 * @version $Revision: #3 $
...
{
    ECSStationColourHelper::ECSStationColourHelper(const ta_uint32 key)
        :
        m_name(""),
        m_key(key),
        m_dateCreated(0),
    ...
    ECSStationColourHelper::ECSStationColourHelper(const ta_uint32 row, TA_Base_Core::IData& data)
        :
        m_isValidData(false)
    {
        FUNCTION_ENTRY("ECSStationColourHelper(const ta_uint32 , TA_Base_Core::IData&)");
    ...
    void ECSStationColourHelper::setStationColourKey(ta_uint32 colour, IECSStationColourData::EColourType type)
    {
        if (! m_isValidData)
        {
            reload();
        ...
        std::map<IECSStationColourData::EColourType, ta_uint32>::iterator matching;
        ...
        {
            m_ecsStationColourKey.insert(std::map<IECSStationColourData::EColourType, ta_uint32>::value_type(type, colour));
        }
    }

    ta_uint32 ECSStationColourHelper::getKey()
    {
        return m_key;
    }
    ...
    ta_uint32 ECSStationColourHelper::getStationColourKey(IECSStationColourData::EColourType type)
    {
        if (! m_isValidData)
        {
            reload();
    ...
    {
        FUNCTION_ENTRY("ECSStationColourHelper::invalidate");
        m_isValidData = false;
    }
    ...
    {
        FUNCTION_ENTRY("ECSStationColourHelper::reload");

        // get a connection to the database
        IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase(Ecs_SCd/*Ecs_Cd*/, Read);
        ...
        if (NULL == data.get() || 0 == data->getNumRows()) // No entry found with the specified pkey
        {
            ...
            sprintf(reasonMessage, "No ECS Station Colour found for pkey = %u", m_key);
    ...
    void ECSStationColourHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data)
    {
        ...
        m_ecsStationColourKey.insert(std::map<IECSStationColourData::EColourType, ta_uint32>::value_type(
