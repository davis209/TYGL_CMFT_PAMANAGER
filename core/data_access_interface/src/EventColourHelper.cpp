 ...
 * in any form.
 *
 * Source: $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/EventColourHelper.cpp $
 * @author Derrick Liew
 * @version $Revision: #4 $
...
namespace TA_Base_Core
{
    ...
    EventColourHelper::EventColourHelper(ta_uint32 key)
        :
        m_name(""),
        m_key(key),
        m_dateCreated(0),
    ...
    EventColourHelper::EventColourHelper(const ta_uint32 row, TA_Base_Core::IData& data)
        :   m_name(""),
            m_isValidData(false)
    {
        FUNCTION_ENTRY("EventColourHelper(const ta_uint32 , TA_Base_Core::IData&)");
    ...
    void EventColourHelper::setEventColourKey(ta_uint32 colour, IEventColourData::EEventColourType type)
    {
        if (! m_isValidData)
        {
            reload();
        ...
        std::map<IEventColourData::EEventColourType, ta_uint32>::iterator matching;
        ...
        {
            m_eventColourKey.insert(std::map<IEventColourData::EEventColourType, ta_uint32>::value_type(type, colour));
        }
    }

    ta_uint32 EventColourHelper::getKey()
    {
        return m_key;
    }
    ...
    ta_uint32 EventColourHelper::getEventColourKey(IEventColourData::EEventColourType type)
    {
        if (! m_isValidData)
        {
            reload();
    ...
    void EventColourHelper::reload()
    {
        ...
        if (NULL == data.get() || 0 == data->getNumRows()) // No entry found with the specified pkey
        {
            ...
            sprintf(reasonMessage, "No event colour found for pkey = %u", m_key);
    ...
    void EventColourHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data)
    {
        ...
        m_eventColourKey.insert(std::map<IEventColourData::EEventColourType, ta_uint32>::value_type(
    ...
    void EventColourHelper::applyChanges()
    {
        ...
        // Execute the query. The method can throw a DatabaseException.
        // This is documented in the comment of this method.
        IDatabase* databaseConnectionWrite = DatabaseFactory::getInstance().getDatabase(/*Event_SCd*/Event_Ad, Write);
