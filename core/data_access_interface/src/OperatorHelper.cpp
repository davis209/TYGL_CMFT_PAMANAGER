 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/OperatorHelper.cpp $
 * @author:  Nick Jardine
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Core
{
    ...
    const ta_uint32 OperatorHelper::DEFAULT_NAME_MAX_LENGTH = 85;
    const ta_uint32 OperatorHelper::DEFAULT_DESCRIPTION_MAX_LENGTH = 80;
    volatile ta_uint32 OperatorHelper::m_nameMaxLength = 0;
    ...
    OperatorHelper::OperatorHelper(const ta_uint32 key)
        : m_key(key),
          m_dateCreated(0),
          m_dateModified(0),
          m_loadProfiles(true),
    ...
    OperatorHelper::OperatorHelper(ta_uint32 row, TA_Base_Core::IData& data, std::vector<ta_uint32>& profiles, std::vector<ta_uint32>& regions)
        :
        m_loadProfiles(false),
        m_loadRegions(false),
        m_isValidData(false),
        ...
        m_isNew(false)
    {
        FUNCTION_ENTRY("OperatorHelper(const ta_uint32 , TA_Base_Core::IData&)");
    ...
    ta_uint32 OperatorHelper::getKey()
    {
    ...
    {
        FUNCTION_ENTRY("OperatorHelper::setName");
    ...
    {
        FUNCTION_ENTRY("OperatorHelper::setDescription");
    ...
    std::string OperatorHelper::getPassword(const std::string& options)
    {
        if (stdutil::icontains_any(options, "--reload", "--force", "--force-reload") || (!m_isValidData && !m_isNew))
        {
            reload();
        }
    ...
    {
        FUNCTION_ENTRY("OperatorHelper::setPassword");
    ...
    void OperatorHelper::setSALT(const std::string strSALT)
    {
        FUNCTION_ENTRY("OperatorHelper::setSALT");
    ...
    std::vector<IProfile*> OperatorHelper::getAssociatedProfiles()
    {
        ...
        std::vector<ta_uint32> keys = getAssociatedProfileKeys();
        std::vector<IProfile*> ret;

        std::vector<ta_uint32>::const_iterator it;
    ...
    std::vector<ta_uint32> OperatorHelper::getAssociatedRegionKeys()
    {
    ...
    std::vector<ta_uint32> OperatorHelper::getAssociatedProfileKeys()
    {
    ...
    void OperatorHelper::setAssociatedProfile(ta_uint32 profileKey, bool isAssociated)
    {
        FUNCTION_ENTRY("OperatorHelper::setAssociatedProfile");
        ...
        std::vector<ta_uint32>::iterator matchingProfile = std::find(m_associatedProfiles.begin(), m_associatedProfiles.end(), profileKey);
    ...
    {
        FUNCTION_ENTRY("OperatorHelper::invalidate");
    ...
    {
        FUNCTION_ENTRY("OperatorHelper::reload");
        ...
        IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase({OnlineUpdatable_Cd, OnlineUpdatable_Cached}, Read);
        ...
        if (!m_isValidProfileData && m_loadProfiles)
        {
            ...
            do
            {
                for (ta_uint32 i = 0; i < data2->getNumRows(); i++)
                {
                    try
                    {
                        ta_uint32 profileId  = data2->getUnsignedLongData(i, columnNames[0]);
        ...
        if (!m_isValidRegionData && m_loadRegions)
        {
            ...
            do
            {
                for (ta_uint32 i = 0; i < data2->getNumRows(); i++)
                {
                    try
                    {
                        ta_uint32 regionId  = data2->getUnsignedLongData(i, columnNames[0]);
    ...
    void OperatorHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data, std::vector<ta_uint32>& profiles, std::vector<ta_uint32>& regions)
    {
    ...
    {
        FUNCTION_ENTRY("OperatorHelper::reloadProfiles");
    ...
    {
        FUNCTION_ENTRY("OperatorHelper::reloadRegions");
    ...
    {
        FUNCTION_ENTRY("OperatorHelper::writeOperatorData");
    ...
    {
        FUNCTION_ENTRY("OperatorHelper::deleteEntity");
        ...
        SQLStatement sql;
        databaseConnection->prepareSQLStatement(sql, 
                                                OPERATOR_UPDATE_6151, 
                                                getLocalDatabaseName(),
                                                databaseConnection->escapeInsertString(newOperatorName), 
                                                databaseConnection->escapeInsertString(newDescription), 
                                                m_key);
        ...
        databaseConnection->prepareSQLStatement(sql, SE_OPERATOR_PROFILE_DELETE_5401, getLocalDatabaseName(), m_key);
    ...
    std::vector<ta_uint32> OperatorHelper::getProfileKeys()
    {
        // get a connection to the database
        ...
        std::vector<ta_uint32> profileKeys;
    ...
    void OperatorHelper::updateProfileAssociations()
    {
        ...
        std::vector<ta_uint32> profileKeys = getProfileKeys();
        std::vector<ta_uint32> localProfilesToAdd = m_associatedProfiles;
        std::vector<ta_uint32> databaseProfilesToRemove;
        ...
        {
            std::vector<ta_uint32>::iterator matching = std::find(localProfilesToAdd.begin(), localProfilesToAdd.end(), profileKeys[i]);
        ...
        if (!databaseProfilesToRemove.empty())
        {
            ...
            for (std::vector<ta_uint32>::iterator iterToRemove = databaseProfilesToRemove.begin();
            ...
            SQLStatement sql;
            databaseConnection->prepareSQLStatement(sql, SE_OPERATOR_PROFILE_DELETE_5402, getLocalDatabaseName(), m_key, sqlVar.str());
        ...
        {
            for (std::vector<ta_uint32>::iterator iterToAdd = localProfilesToAdd.begin();
                    iterToAdd != localProfilesToAdd.end(); ++iterToAdd)
            {
                ...
                SQLStatement sql;
                databaseConnection->prepareSQLStatement(sql, SE_OPERATOR_PROFILE_INSERT_5351, getLocalDatabaseName(),
                                                        databaseConnection->getNextPkeyDynamic("SECURITY_SEQ"), m_key, *iterToAdd);
    ...
    void OperatorHelper::modifyExistingOperator()
    {
        ...
        LOG_DEBUG("Operator %s already exists. It's data will be updated.", m_name);
    ...
    void OperatorHelper::addNewOperator()
    {
        ...
        LOG_DEBUG("Operator %s is a new operator. It will be added to the database.", m_name);
        ...
        databaseConnection->prepareSQLStatement(sql, OPERATOR_V_INSERT_6851,
												getLocalDatabaseName(),
                                                databaseConnection->getNextPkeyDynamic("OPERATOR_SEQ"),
                                                ...
                                                databaseConnection->escapeInsertString(m_salt));
    ...
    std::string OperatorHelper::retrieveDeletedField(const std::string& currentField, const std::string& columnName, ta_uint32 maxFieldSize)
    {
