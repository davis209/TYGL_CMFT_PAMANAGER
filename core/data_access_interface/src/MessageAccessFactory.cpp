 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/MessageAccessFactory.cpp $
 * @author:  Nick Jardine
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Core
{
    ...
    const ta_uint32 MessageAccessFactory::INVALID_SEQUENCE_NUMBER(0);
    const ta_uint32 MessageAccessFactory::MIN_SEQUENCE_NUMBER(1);
    const ta_uint32 MessageAccessFactory::MAX_SEQUENCE_NUMBER(4000000000);
    const ta_uint32 MessageAccessFactory::MAX_ROWS(20000);
    ...
    std::vector<IMessageData*> MessageAccessFactory::getAuthMessagesForSession(const std::string& session)
    {
        ...
        SQLStatement strSql;
        databaseConnection->prepareSQLStatement(strSql, EVENT_SELECT_17051, session, m_sessionMemtypIDs);
    ...
    //                                                                    ta_uint32 precedingMsgPkey, ta_uint32 maxMsgs)
    //     {
    ...
    //         TA_ASSERT(columnNames.size() <= static_cast<ta_uint32>(SimpleDb::MAX_COLS), "Too many message view column names have been specified");
    //
    //         IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase(Event_Ad, Read);
    //
    ...
    //             << "    AND ROWNUM <= " << maxMsgs
    //          << " ORDER BY PKEY";*/
    //      const char *pszViewName = boost::to_lower_copy(const_cast<char*>(databaseConnection->escapeQueryString(viewName).c_str()));
    //
    ...
    //         if (msgCount < maxMsgs)
    //         {
    //              ta_uint32 nextMsgPkey = getNextMessageSequenceNumber();
    //
    ...
    //      ta_uint32 MessageAccessFactory::getNextMessageSequenceNumber()
    //     {
    ...
    //
    //         std::auto_ptr<IData> data(DatabaseFactory::getInstance().getDatabase(Event_Ad, Read)->executeQuery(sql, columnNames));
        ...
        : m_stopLoad(false), m_pauseLoad(false), m_operatorName("")
    {
        try
        {
            ...
            std::string strTemp;
            strTemp += "'" + SESSION_STARTED_EVENT_NAME + "',";
            strTemp += "'" + SESSION_ENDED_EVENT_NAME + "',";
            strTemp += "'" + OVERRIDE_STARTED_EVENT_NAME + "',";
            strTemp += "'" + OVERRIDE_ENDED_EVENT_NAME + "'";

            IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase(Event_SCd, Read);
            SQLStatement strSql;
            databaseConnection->prepareSQLStatement(strSql, EVENT_SELECT_17354, strTemp);

            // Set up the columnNames vector to be passed to executeQuery()
            std::string message_id = "memtyp_id";
            ...
            for (unsigned int i = 0; i < data->getNumRows(); i++)
            {
                ta_uint32 id = data->getUnsignedLongData(i, message_id);
    ...
    //TD 15056
    //zhou yuan++
    //  std::vector<IMessageData*> MessageAccessFactory::getMessageOfEntity( const ta_uint32 entityKey,
