 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/GlobalParameterAccessFactory.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #4 $
 *
...
namespace TA_Base_Core
{
    ...
    std::vector<IGlobalParameter*> GlobalParameterAccessFactory::getAllGlobalParametersForSubsystem(ta_uint32 subsystemKey, const bool readWrite)
    {
    ...
    IGlobalParameter* GlobalParameterAccessFactory::getGlobalParameterByKey(const ta_uint32 GlobalParameterKey, const bool readWrite)
    {
        ...
        const bool readWrite)
    {
        ...
        databaseConnection->prepareSQLStatement(strSql, GLOBAL_PARAMETER_SELECT_16054,
                                                databaseConnection->escapeInsertString(boost::to_upper_copy(name)), subsystemKey);
        ...
        sql << " select GPARAM_ID, NAME, VALUE, SUBSYSTEMKEY,"
            << " TO_CHAR(DATE_MODIFIED,'YYYYMMDDHH24MISS'),"
            << " TO_CHAR(DATE_CREATED,'YYYYMMDDHH24MISS') FROM GLOBAL_PARAMETER"
            << " where UPPER(NAME) = '" << databaseConnection->escapeInsertString(boost::to_upper_copy(name)) << "'"
    ...
    ta_uint32 GlobalParameterAccessFactory::getGlobalParameterKeyFromNameAndSubsystem(
        const std::string& name,
        ta_uint32 subsystemKey)
    {
