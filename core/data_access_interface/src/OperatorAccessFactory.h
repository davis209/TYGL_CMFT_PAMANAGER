 ...
 * in any form.
 *
 * Source: $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/OperatorAccessFactory.h $
 * @author Nick Jardine
 * @version $Revision: #3 $
...
namespace TA_Base_Core
{
    ...
    public:
         ...
         */
        IOperator* getOperator(const ta_uint32 key, const bool readWrite);

        /**
         * getOperatorSaltValue
         *
         * Get the operator salt value given the operator key.
         *
         * @param key The database key to the operator to retrieve
         *
         * @return true if the salt value returned successfully.
         *
         * @exception DatabaseException A DatabaseException will be thrown if there are
         ...
         *            1) Invalid connection string (as stored in RunParams)
         *            2) Database is not/cannot be opened
         *            3) Error while attempting to execute a database query (i.e. the column does not exists)
         * @exception DataException A DataException will be thrown if there is no operator
         *            matching the supplied PKEY, or if there is more than one operator
         *            matching the supplied PKEY.
         */
        bool getOperatorSaltValue(const ta_uint32 key, std::string& saltValue);
        ...
        std::vector<ta_uint32>& getProfileByOperator(ta_uint32 operatorId);
        std::vector<ta_uint32>& getRegionByOperator(ta_uint32 operatorId);
    ...
    private:
        ...
        typedef std::map< ta_uint32, std::vector<ta_uint32> > OperatorMap;

        std::once_flag m_operatorToProfileLoad;
        OperatorMap m_operatorToProfileMap;
        NonReEntrantThreadLockable m_operatorToProfileLock;
        std::once_flag m_operatorToRegionLoad;
