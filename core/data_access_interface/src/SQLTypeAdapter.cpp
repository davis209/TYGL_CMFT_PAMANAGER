    ...
    , m_bEmpty(true)
{
    ...
    else
    {
        ...
        m_strParams[enumSqliteDb] = rhs.strSQLite;
    }
}
...
void SQLTypeAdapter::add(int nDynamicKey)
{
    if (!m_strParams[enumBeginDbType].empty())
    {
        ...
        m_strParams[enumSqliteDb].append(m_strParams[enumBeginDbType]); // add SQLite
    ...
    m_strParams[enumSqliteDb].append(DynFormatData.strSqlite);  // add SQLite
}
...
void SQLTypeAdapter::add(const std::string& strVal)
{
    ...
    if (enumBeginDbType != m_nDataCategory)
    {
        ...
        m_strParams[enumSqliteDb].append(strVal);
    }
    else
    {
...
void SQLTypeAdapter::add(const std::string& strOracle, const std::string& strMysql)
{
    ...
    if (!m_strParams[enumBeginDbType].empty())
    {
        m_strParams[enumOracleDb].append(m_strParams[enumBeginDbType]);
        m_strParams[enumMysqlDb].append(m_strParams[enumBeginDbType]);
        m_strParams[enumSqliteDb].append(m_strParams[enumBeginDbType]); // This function only for dynamic data
...
void SQLTypeAdapter::add(const std::string& strColunm, const std::string& strVal, int nColType, const char* strOperator, bool bIsAQ)
{
    ...
    if (enumBeginDbType != m_nDataCategory)
    {
        ...
        else if (enumValType_STR == nColType)
        {
            ...
            m_strParams[enumSqliteDb].append(s_strNonAQTag + strVal + s_strNonAQTag);
        }
        else
        {
            m_strParams[enumOracleDb].append(strColunm + strOperator + strVal);
            m_strParams[enumMysqlDb].append(strColunm + strOperator + strVal);
            m_strParams[enumSqliteDb].append(strColunm + strOperator + strVal);
        }
    }
    else
    {
...
void SQLTypeAdapter::add(const Row& vecVal, int nDynKey, bool bIsLast)
{
    ...
    if (!m_strParams[enumBeginDbType].empty())
    {
        m_strParams[enumOracleDb].append(m_strParams[enumBeginDbType]);
        m_strParams[enumMysqlDb].append(m_strParams[enumBeginDbType]);
        m_strParams[enumSqliteDb].append(m_strParams[enumBeginDbType]);
    ...
    m_strParams[enumOracleDb].append(DynFormatData.strOracl);
    m_strParams[enumMysqlDb].append(DynFormatData.strMysql);
    m_strParams[enumSqliteDb].append(DynFormatData.strSqlite);
...
void SQLTypeAdapter::add(int nFuncKey, const std::string& strColunm, int nOperKey)
{
    ...
    m_strParams[enumSqliteDb].append(strColunm + strOper + DynFunData.strSqlite);
}
                         ...
                         int nFuncKey, const char* strOperator, bool bIsAQ)
{
    ...
    m_strParams[enumSqliteDb].append(strColunm + strOper);
    ...
    m_strParams[enumSqliteDb].append(DynFunData.strSqlite + s_strFuncLeftBegin);
    ...
    if (bIsAQ)
    {
        ...
        m_strParams[enumSqliteDb].append(s_strAQTag + DynFormatData.strSqlite + s_strAQTag);
        m_strParams[enumOracleDb].append(s_strFuncRightEnd);
        m_strParams[enumMysqlDb].append(s_strFuncRightEnd);
        m_strParams[enumSqliteDb].append(s_strFuncRightEnd);
    }
    else
    {
        ...
        m_strParams[enumSqliteDb].append(s_strNonAQTag + DynFormatData.strSqlite + s_strNonAQTag);
        m_strParams[enumOracleDb].append(s_strFuncRightEnd);
        m_strParams[enumMysqlDb].append(s_strFuncRightEnd);
        m_strParams[enumSqliteDb].append(s_strFuncRightEnd);
    }
...
// anton.qiu: use a separate function for date function, as the order of parameter can be different in different database
void SQLTypeAdapter::addDateFunc(const std::string& strColunmName, int nDateFormatKey, bool bIsAQ)
{
    if (!m_strParams[enumBeginDbType].empty())
    {
        m_strParams[enumOracleDb].append(m_strParams[enumBeginDbType]);
    ...
    DynamicData DynFormatData, DynFunData;
    CSQLParamsHelper sqlParaHelper;
    sqlParaHelper.getDynamicSQLPara(nDateFormatKey, DynFormatData);
    sqlParaHelper.getDynamicSQLPara(enumFuncFormat_10, DynFunData);
    ...
    if (bIsAQ)
    {
        ...
        // First parameter
        m_strParams[enumOracleDb].append(strColunmName + s_strDelimit1);    // Oracle: Column Name
        m_strParams[enumMysqlDb].append(strColunmName + s_strDelimit1);     // MySQL: Column Name
        m_strParams[enumSqliteDb].append(s_strAQTag + DynFormatData.strSqlite + s_strAQTag);    // Note, for SQLite, date format first

        // Second parameter
        m_strParams[enumOracleDb].append(s_strAQTag + DynFormatData.strOracl + s_strAQTag); // Oracle: DateFormat
        m_strParams[enumMysqlDb].append(s_strAQTag + DynFormatData.strMysql + s_strAQTag);  // MySQL: DateFormat
        m_strParams[enumSqliteDb].append(strColunmName + s_strDelimit1);    // SQLite: ColumnName
    ...
    else
    {
        ...
        m_strParams[enumOracleDb].append(strColunmName + s_strDelimit1);    // Oracle: Column Name
        m_strParams[enumMysqlDb].append(strColunmName + s_strDelimit1);     // MySQL: Column Name
        m_strParams[enumSqliteDb].append(s_strNonAQTag + DynFormatData.strSqlite + s_strNonAQTag);  // Note, for SQLite, date format first

        // Second parameter
        m_strParams[enumOracleDb].append(s_strNonAQTag + DynFormatData.strOracl + s_strNonAQTag);   // Oracle: DateFormat
        m_strParams[enumMysqlDb].append(s_strNonAQTag + DynFormatData.strMysql + s_strNonAQTag);    // MySQL: DateFormat
        m_strParams[enumSqliteDb].append(strColunmName + s_strDelimit1);    // SQLite: ColumnName
...
void SQLTypeAdapter::add(const std::string& strColunm, int nDynaKey, int nFuncKey, bool bIsAQ)
{
    ...
    if (bIsAQ)
    {
        ...
        m_strParams[enumSqliteDb].append(strColunm + s_strDelimit1);
        m_strParams[enumOracleDb].append(s_strAQTag + DynFormatData.strOracl + s_strAQTag);
        m_strParams[enumMysqlDb].append(s_strAQTag + DynFormatData.strMysql + s_strAQTag);
        m_strParams[enumSqliteDb].append(s_strAQTag + DynFormatData.strSqlite + s_strAQTag);
        m_strParams[enumOracleDb].append(s_strFuncRightEnd);
...
void SQLTypeAdapter::add(SQLStatement& rSql, const std::string& strWhere)
{
    ...
    else
    {
		...
		rSql.strSQLite.append(strWhere);
    }
}
...
void SQLTypeAdapter::_AddData(const SQLTypeAdapter& other)
{
    ...
    if (enumBeginDbType != other.getDataCategory())
    {
        ...
        m_strParams[enumSqliteDb].append(other.data(enumSqliteDb));
    }
    else
    {
        if (enumBeginDbType != m_nDataCategory)
        {
            ...
            m_strParams[enumSqliteDb].append(other.data(enumBeginDbType));
        }
        else
        {
