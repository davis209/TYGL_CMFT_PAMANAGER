 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/ConfigAlarmType.cpp $
 * @author:  Nick Jardine
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Core
{
    ...
    ta_int32 ConfigAlarmType::s_nextAvailableIdentifier = 0;

    ConfigAlarmType::ConfigAlarmType(ta_uint32 key)
        :
    ...
    ConfigAlarmType::ConfigAlarmType(const ta_uint32 row, TA_Base_Core::IData& data)
        :
    ...
    ta_uint32 ConfigAlarmType::getUniqueIdentifier()
    {
    ...
    void ConfigAlarmType::setSeverityKey(const ta_uint32 severityKey)
    {
        TA_ASSERT(m_helper != NULL,
                  "Attempt to change state when object has been deleted.");

        ta_uint32 oldSeverityKey = m_helper->getSeverityKey();
    ...
    std::string ConfigAlarmType::getSeverityName(const ta_uint32 key)
    {
    ...
    ta_uint32 ConfigAlarmType::getContextKey()
    {
    ...
    ta_uint32 ConfigAlarmType::getKey()
    {
        TA_ASSERT(m_helper != NULL,
                  "Attempt to change state when object has been deleted.");
    ...
    ta_uint32 ConfigAlarmType::getSeverityKey()
    {
        TA_ASSERT(m_helper != NULL,
                  "Attempt to change state when object has been deleted.");
    ...
    void ConfigAlarmType::setRelatedMMSType(IAlarmData::EMmsState mmsState)
    {
        ...
        updateAlarmTypeChanges(
            MMS_STATE,
            getUnsignedLongAsString((ta_int32)oldmmsState),
            getUnsignedLongAsString((ta_int32)mmsState));
    }
    ...
    std::string ConfigAlarmType::getUnsignedLongAsString(ta_uint32 number)
    {
        char buff[256] = {0};
    ...
    ta_uint32 ConfigAlarmType::getUnsignedLongFromString(const std::string& theString)
    {
