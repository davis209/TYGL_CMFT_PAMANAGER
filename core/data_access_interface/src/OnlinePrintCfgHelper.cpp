 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/OnlinePrintCfgHelper.cpp $
 * @author:  Nick Jardine
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Core
{
    ...
    OnlinePrintCfgHelper::OnlinePrintCfgHelper(const ta_uint32 idKey)
        :
        m_defaultPrinter(""),
        m_location(idKey),
        m_isValidData(false),
    ...
    OnlinePrintCfgHelper::OnlinePrintCfgHelper(ta_uint32 row, TA_Base_Core::IData& data)
        :
        m_defaultPrinter(""),
        m_location(0),
        m_isValidData(false),
    ...
    ta_uint32 OnlinePrintCfgHelper::getLocation()
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    void OnlinePrintCfgHelper::setLocation(ta_uint32 pLocation)
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    void OnlinePrintCfgHelper::reloadUsing(ta_uint32 row, TA_Base_Core::IData& data)
    {
    ...
    void OnlinePrintCfgHelper::writeOnlinePrintCfgData()
    {
    ...
    void OnlinePrintCfgHelper::modifyExistingOnlinePrintCfg()
    {
        FUNCTION_ENTRY("modifyExistingOnlinePrintCfg");

        LOG_DEBUG("OnlinePrintCfg id=%ul already exists. It's data will be updated.", m_location);
    ...
    void OnlinePrintCfgHelper::addNewOnlinePrintCfg()
    {
