 ...
 * in any form.
 *
 * Source: $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/SimpleDbDatabase.cpp $
 * @author Nick Jardine
 * @version $Revision: #4 $
...
namespace TA_Base_Core
{
    void SimpleDbDatabase::connect(const std::string& connectionString)
    {
        ...
        if (m_db == NULL) // The pointer has not been initialised. We need to set it up.
        {
            ...
            //
            LOG_DEBUG("connect(): Database connection string is: %s", connectionString);

            // Parse the Connection String to seperate the database connection information.
            //
            std::vector<std::string> dbConnData = DatabaseUtil::splittedConnectionStr(connectionString);
            ...
            // as ta_int32 as we have at least three, then there is not a problem.
            ...
            catch (const DBException& e)
            {
                ...
                LOG_EXCEPTION("TA_Base_Core::DBException", e.getSummary());
                FUNCTION_EXIT;
    ...
    void SimpleDbDatabase::disconnect()
    {
        ...
        LOG_CALLSTACK(boost::format("SimpleDbDatabase[%s]::disconnect") % makePrettyCallstackId());
    ...
    void SimpleDbDatabase::cleanQuery()
    {
        ...
        LOG_CALLSTACK(boost::format("SimpleDbDatabase[%s]::cleanQuery") % makePrettyCallstackId());
        ...
        if (m_db)
        {
            ...
            {
                LOG_EXCEPTION("TA_Base_Core::DBException", "database cleanQuery failure");
            }
        }
        else
        {
            LOG_EXCEPTION("TA_Base_Core::DBException", "database is NULL");
        }

        FUNCTION_EXIT;
    }

    std::string SimpleDbDatabase::getSqlString(const SQLStatement& sqlStatement)
    {
        switch (getDbServerType())
        {
            case EDatabaseServerType::Oracle:
                return sqlStatement.strOracle.size() ? sqlStatement.strOracle : sqlStatement.strCommon;
                break;

            case EDatabaseServerType::MySQL:
                return sqlStatement.strMySQL.size() ? sqlStatement.strMySQL : sqlStatement.strCommon;
                break;

            case EDatabaseServerType::SQLite:
                return sqlStatement.strSQLite.size() ? sqlStatement.strSQLite : sqlStatement .strCommon;
                break;
    ...
    IData* SimpleDbDatabase::executeQuery(const SQLStatement& rSqlObj, const std::vector<std::string>& columnNames, const std::string& options)
    {
        return (executeQuery(rSqlObj, columnNames, m_db->getDefaultFetchSize(), options + DATABASE_QUERY_OPTION_RELAX));
    }

    IData* SimpleDbDatabase::executeQuery(const SQLStatement& rSqlObj, const std::vector<std::string>& columnNames, size_t numRows, const std::string& options)
    {
        FUNCTION_ENTRY("executeQuery");
        LOG_CALLSTACK(boost::format("SimpleDbDatabase[%s]::executeQuery:%s") % makePrettyCallstackId() % rSqlObj.strSQLID);
        ...
        bool relax = boost::icontains(options, DATABASE_QUERY_OPTION_RELAX);
        TA_ASSERT(0 < numRows, "numRows is 0 - this is not a sensible number");
        TA_ASSERT(relax || static_cast<unsigned int>(SimpleDb::MAX_ROWS) >= numRows, "numRows is greater than SimpleDb::MAX_ROWS - this is not permissable by SimpleDb");
        ...
        TA_ASSERT(relax || 0 < columnNames.size(), "columnNames is zero size");
        ...
        try
        {
            ...
            m_db->Exec(rSqlObj, columnNames.size(), data, numRows, options);
        ...
        {
            // if we catch anything in here, it means the db in used no ta_int32 available
            // so just record the log, next time the application getDatabase will try another db
            LOG_EXCEPTION("TA_Base_Core::DatabaseException", e.what());
        }

        // Set the member variables to match the passed-in variables
        m_columnNames = columnNames;

        TA_Base_Core::SimpleDbData* idata = nullptr;

        if (boost::icontains(options, DATABASE_QUERY_OPTION_WITH_COLUMN_NAMES))
        {
            idata = new TA_Base_Core::SimpleDbData(std::move(data));
        }
        else
        {
            // Place the data we just loaded into an instance of TA_Base_Core::SimpleDbData (which IS A IData)
            idata = new TA_Base_Core::SimpleDbData(columnNames, std::move(data));
        }

        m_numRows = idata->getNumRows();

        LOG_SQL("executeQuery(): %d rows returned", m_numRows);
    ...
    void SimpleDbDatabase::executeModification(const SQLStatement& rSqlObj)
    {
        ...
        LOG_CALLSTACK(boost::format("SimpleDbDatabase[%s]::executeModification:%s") % makePrettyCallstackId() % rSqlObj.strSQLID);
    ...
    bool SimpleDbDatabase::moreData(IData*& returnData)
    {
        ...
        auto& id = m_db->getStatement().strSQLID;
        LOG_CALLSTACK(boost::format("SimpleDbDatabase[%s]::moreData%s") % makePrettyCallstackId() % (id.size() ? ":" + id : ""s));
        ...
        if (! m_db->IsOpen())
        {
            ...
            {
                tryToOpenDb();
                LOG_WARN("moreData(): Database should be opened while requesting more data,something wrong may occurred");
            }
            catch (DatabaseException&)
            {
                LOG_ERROR("moreData(): Database exception or crash down");
                return false;
            }
            catch (...)
            {
                LOG_ERROR("moreData(): Unknown Database exception");
                return false;
            }
        }
        ...
        if (true == m_db->HasMore()) // There is data to read
        {
            ...
            try
            {
                LOG_DATABASE("moreData(): calling FetchMore...");
                m_db->FetchMore(m_columnNames.size(), data, SimpleDb::MAX_ROWS);
                LOG_SQL("moreData(): %d rows returned", data.size());
            }
            catch (const DBException& e)
            {
                LOG_EXCEPTION("TA_Base_Core::DBException", e.getSummary());
            ...
            returnData = new TA_Base_Core::SimpleDbData(m_columnNames, std::move(data));
            FUNCTION_EXIT;
            return true;
        }
        else
    ...
    {
        LOG_CALLSTACK(boost::format("SimpleDbDatabase[%s]::readBLOB") % makePrettyCallstackId());
        ...
        catch (const DBException& e)
        {
            ...
            catch (...)
            {
                ...
                LOG_ERROR("Failed to read BLOB filed %s from table %s with clause %s", fieldName, tableName, whereClause);
    ...
    void SimpleDbDatabase::writeBLOB(const std::string& fieldName, const std::string& tableName,
                                     const std::string& whereClause, std::vector<unsigned char>& data)
    {
        LOG_CALLSTACK(boost::format("SimpleDbDatabase[%s]::writeBLOB") % makePrettyCallstackId());
        ...
        catch (const DBException& e)
        {
            ...
            catch (...)
            {
                ...
                LOG_ERROR("Failed to write BLOB filed %s to table %s with clause %s", fieldName, tableName, whereClause);
    ...
    EDatabaseServerType SimpleDbDatabase::getDbServerType()
    {
    ...
    void SimpleDbDatabase::prepareSQLStatement(SQLStatement& rSqlStatement, const SQLVarParms& varParms)
    {
        FUNCTION_ENTRY("prepareSQLStatement");
        LOG_CALLSTACK(boost::format("SimpleDbDatabase[%s]::prepareSQLStatement:%s") % makePrettyCallstackId() % varParms[0].data(enumBeginDbType));

        try
        {
            LOG_SQL("Begin prepare SQL statement...");
            //std::string strSQL;
            ...
            LOG_SQL("End prepare SQL statement.");
            //return strSQL;
        }
        catch (BadParamCount* e)
        {
            TA_THROW_INFO(SQLCodeException(e->what()));
        }
        catch (BadIndex* e)
        {
            TA_THROW_INFO(SQLCodeException(e->what()));
        ...
        {
            TA_THROW_INFO(SQLCodeException("Unknown SQLCode exception"));
        }
    ...
    void SimpleDbDatabase::ParserCallPkgString(std::string& str)
    {
        ...
        str = str + ";";
        LOG_SQL("ParserCallPkgString returned: %s", str);
    }
    ...
    void SimpleDbDatabase::ParserSQLString(std::string& str)
    {
        ...
        LOG_SQL("ParserSQLString returned: %s", str);
    }

	//Will cater to static db connection
    std::string SimpleDbDatabase::getNextPkey(const std::string& strSeqName)
    {
        std::vector<std::string> columns;
        columns.push_back("LAST_NUMBER");
        TA_Base_Core::SQLStatement strSql;
        TA_Base_Core::IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase(TA_Base_Core::OnlineUpdatable_Cd, TA_Base_Core::Read);

		//execute statement to update latest last number
        databaseConnection->prepareSQLStatement(strSql, NEXTVAL_UPDATE_83002, strSeqName, strSeqName, strSeqName);
        databaseConnection->executeQuery(strSql, columns);		

		//get the last number
		databaseConnection->prepareSQLStatement(strSql, NEXTVAL_SELECT_83003, strSeqName);
        ...
        strRet << data->getUnsignedLongData(0, "LAST_NUMBER");

        return strRet.str();
    }

	//Will cater to dynmic db connection
	//Added because adding operator will use dynamic DB
	//hongzhi, for TDS
	std::string SimpleDbDatabase::getNextPkeyDynamic(const std::string& strSeqName)
	{
		std::vector<std::string> columns;
    ...
    const std::string& SimpleDbDatabase::makePrettyCallstackId()
    {
        static CacheDecorator<std::string, std::string> s_func = [](auto & str)
        {
            auto o = DatabaseUtil::getConnectionOptions(str);

            switch (o->serverType)
            {
                case EDatabaseServerType::MySQL:
                case EDatabaseServerType::CacheDB:
                    return o->serverTypeStr + ":" + o->databaseName + "@" + o->hostName;

                case EDatabaseServerType::Oracle:
                case EDatabaseServerType::SQLite:
                default:
                    return o->serverTypeStr + ":" + o->databaseName;
            }
        };

        static const std::string s_empty = "<NULL>";
        return m_db ? s_func(m_db->getConnectionStr()) : s_empty;
    }
}
