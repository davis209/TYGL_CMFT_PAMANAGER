 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/ProfileHelper.cpp $
...
namespace TA_Base_Core
{
    ...
    const ta_uint32 ProfileHelper::NORMAL_SUBSYSTEM_STATE(0);
    const ta_uint32 ProfileHelper::DEFAULT_NAME_MAX_LENGTH = 85;
    const ta_uint32 ProfileHelper::INVALID_ACCESS_CONTROL_GROUP_KEY = 0xFFFFFFFF;
    const ta_uint32 ProfileHelper::CENTRAL_PROFILE_TYPE = 1;
    const ta_uint32 ProfileHelper::LOCAL_PROFILE_TYPE = 0;
    volatile ta_uint32 ProfileHelper::m_nameMaxLength = 0;
    TA_Base_Core::NonReEntrantThreadLockable ProfileHelper::m_nameMaxLengthLock;

    ProfileHelper::ProfileHelper(const ta_uint32 key)
        : m_isValidData(false),
    ...
    ProfileHelper::ProfileHelper(const ta_uint32 row, TA_Base_Core::IData& data)
        :
        m_isValidData(false),
        m_isValidDisplayData(false),
        m_isValidAccessibleSubsystemData(false),
    ...
    ta_uint32 ProfileHelper::getKey()
    {
    ...
    {
        FUNCTION_ENTRY("ProfileHelper::setName");
    ...
    {
        FUNCTION_ENTRY("ProfileHelper::setDisplayName");
    ...
    ta_uint32 ProfileHelper::getType()
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    std::string ProfileHelper::getTypeName(ta_uint32 typeKey)
    {
        ...
        if (m_profileTypes.empty())
        {
            ...
            TA_Base_Core::IDatabase* pDb = DatabaseFactory::getInstance().getDatabase({ OnlineSecurity_Cd, OnlineSecurity_Cached}, Read);
            ...
            try
            {
                do
                {
                    ...
                    {
                        ta_uint32 key = data->getUnsignedLongData(i, "key");
                        std::string name = data->getStringData(i, "name");
                        m_profileTypes.insert(std::map<ta_uint32, std::string>::value_type(key, name));
                    }
        ...
        std::map<ta_uint32, std::string>::iterator matchingType = m_profileTypes.find(typeKey);
    ...
    void ProfileHelper::setType(const ta_uint32 type)
    {
    ...
    std::string ProfileHelper::getDefaultDisplay(ta_uint32 locationKey, int displayNumber)
    {
    ...
    std::string ProfileHelper::getParameterValue(ta_uint32 locationKey, const std::string& parameterName)
    {
    ...
    std::string ProfileHelper::getParameterActualValue(ta_uint32 locationKey, const std::string& parameterName)
    {
        if (!m_isValidDisplayData && !m_isNew)
        {
            reloadDisplayData();
    ...
    ta_uint32 ProfileHelper::getDisplayKey(ta_uint32 locationKey, int displayNumber)
    {
        // This method cannot be called until the name has been set
        std::ostringstream displayName;
    ...
    ta_uint32 ProfileHelper::getDisplayKey(ta_uint32 locationKey, const std::string& parameterName)
    {
        if (!m_isValidDisplayData && !m_isNew)
        {
            reloadDisplayData();
    ...
    void ProfileHelper::setDisplay(ta_uint32 location, ta_uint32 screenNumber, ta_int32 display)
    {
    ...
    void ProfileHelper::setParameter(ta_uint32 location, const std::string& parameterName, const std::string& parameterValue)
    {
    ...
    void ProfileHelper::removeDisplay(ta_uint32 location, ta_uint32 screenNumber)
    {
    ...
    void ProfileHelper::removeParameter(ta_uint32 location, const std::string& parameterName)
    {
        FUNCTION_ENTRY("removeDisplay");
    ...
    std::vector<ta_uint32> ProfileHelper::getAssociatedLocationKeys()
    {
    ...
    std::string ProfileHelper::getActionGroupAsStringForResource(ta_uint32 resourceKey)
    {
        TA_ASSERT(!m_isNew, "m_key has not been set");
        ...
        // anton.qiu: read the static part first
        TA_Base_Core::IDatabase* pDb = DatabaseFactory::getInstance().getDatabase(OnlineUpdatable_Cd, Read);
        SQLStatement strSqlStatic;
        pDb->prepareSQLStatement(strSqlStatic, SE_RESOURCE_V_SELECT_11513, resourceKey);
        std::vector<std::string> colsStatic;
        colsStatic.push_back("subsystem_keys");
        std::auto_ptr<TA_Base_Core::IData> dataStatic(pDb->executeQuery(strSqlStatic, colsStatic));

        // not finding an entry is NOT an error
        if ((NULL == dataStatic.get()) || (0 == dataStatic->getNumRows()))
        {
            return "";
        }

        TA_ASSERT(1 == dataStatic->getNumRows(), "There should be only one or none as PKEY is unique!");
        std::string subsystemkey = dataStatic->getStringData(0, colsStatic[0]);

        // handle if the subsystemkey is empty string
        subsystemkey = boost::trim_left_copy(subsystemkey);
        ...
        pDb = DatabaseFactory::getInstance().getDatabase({ OnlineSecurity_Cd, OnlineSecurity_Cached}, Read);
        ...
        pDb->prepareSQLStatement(strSql, SE_ACTION_GROUP_SELECT_11507, subsystemkey, NORMAL_SUBSYSTEM_STATE, m_key);
        ...
        ta_uint32 subsystemKey = data->getUnsignedLongData(0, cols[1]);
    ...
    std::string ProfileHelper::getActionGroupAsStringForSubsystem(ta_uint32 subsystemKey,
    ...
    bool ProfileHelper::getActionGroup(ta_uint32 subsystem, ta_uint32 subsystemState,
    ...
    ta_uint32 ProfileHelper::getAccessControlGroup()
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    std::string ProfileHelper::getAccessControlGroupName(ta_uint32 accessControlGroupKey)
    {
        FUNCTION_ENTRY("getAccessControlGroupName");

        std::map<ta_uint32, std::string> groupNames;
        ...
        try
        {
            do
            {
                for (unsigned int i = 0; i < data->getNumRows(); ++i)
                {
                    ta_uint32 key = data->getUnsignedLongData(i, "key");
                    std::string name = data->getStringData(i, "name");
                    groupNames.insert(std::map<ta_uint32, std::string>::value_type(key, name));
                }
        ...
        std::map<ta_uint32, std::string>::iterator matchingType = groupNames.find(accessControlGroupKey);
    ...
    std::vector<ta_uint32> ProfileHelper::getAccessibleSubsystemKeys()
    {
    ...
    bool ProfileHelper::isLocationAssociated(ta_uint32 locationId)
    {
        ...
        std::vector<ta_uint32>::iterator result = std::find(m_associatedLocationKeys.begin(),
    ...
    {
        FUNCTION_ENTRY("ProfileHelper::invalidate");
    ...
    void ProfileHelper::writeData()
    {
        ...
        for (std::vector<ta_uint32>::iterator iter_keys = m_associatedLocationKeys.begin(); iter_keys != m_associatedLocationKeys.end(); iter_keys++)
        {
            ProfileParameters pProfileParameters;
            DefaultProfileParamMap::iterator resultIter = m_defaultProfileParams.find((ta_uint32)(*iter_keys));
        ...
        {
            std::vector<ta_uint32>::iterator resultIter = std::find(m_associatedLocationKeys.begin(),
        ...
        {
            TA_THROW(TA_Base_Core::DataConfigurationException("Profile data not fully specified. Profile cannot be written to database", fieldNames));
        }
    ...
    {
        FUNCTION_ENTRY("ProfileHelper::reload");
    ...
    void ProfileHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data)
    {
        ...
        do
        {
            ...
            for (ta_uint32 i = 0; i < numRows; i++)
            {
                ...
                try
                {
                    ...
                    ta_uint32 locationKey = data->getUnsignedLongData(i, locationColumn);

                    ta_uint32 display1 = data->getUnsignedLongData(i, display1Column);
                    ta_uint32 display2 = data->getUnsignedLongData(i, display2Column);
                    ta_uint32 display3 = data->getUnsignedLongData(i, display3Column);
    ...
    void ProfileHelper::modifyExistingProfile()
    {
        ...
        LOG_DEBUG("Profile %s already exists. It's data will be updated.", m_name);
    ...
    void ProfileHelper::addNewProfile()
    {
        ...
        LOG_DEBUG("Profile %s is a new profile. It will be added to the database.", m_name);
        ...
        // Retrieve the pkey as an ta_uint32 - the name of this operator - from the
    ...
    void ProfileHelper::updateLocationDisplays()
    {
        ...
        std::map<ta_uint32, int> locationKeysAlreadyInDatabase;

        do
        {
            for (ta_uint32 i = 0; i < data->getNumRows(); ++i)
            {
                ta_uint32 locationKey = data->getUnsignedLongData(i, locationKeyColumn);

                std::map<ta_uint32, int>::iterator iter = locationKeysAlreadyInDatabase.find(locationKey);
        ...
        for (DefaultProfileParamMap::iterator iter = m_defaultProfileParams.begin(); iter != m_defaultProfileParams.end(); ++iter)
        {
            ...
            if (resultIter != m_associatedLocationKeys.end())
            {
                ...
                std::map<ta_uint32, int>::iterator matchingLocation = locationKeysAlreadyInDatabase.find(iter->first);
    ...
    void ProfileHelper::setAccessControlGroup(ta_uint32 accessControlGroupKey)
    {
    ...
    void ProfileHelper::addLocationAssocation(ta_uint32 locationId)
    {
        ...
        std::vector<ta_uint32>::iterator searchResult = std::find(m_associatedLocationKeys.begin(),
                                                                  m_associatedLocationKeys.end(), locationId);
    ...
    void ProfileHelper::removeLocationAssociaiton(ta_uint32 locationId)
    {
    ...
    std::string ProfileHelper::retrieveDeletedField(const std::string& currentField, const std::string& columnName, ta_uint32 maxFieldSize)
    {
        ...
        ta_uint32 highestAppendedNumber = 0;
        ...
        do
        {
            for (unsigned int i = 0; i < data->getNumRows(); ++i)
            {
                ...
                // Convert the string into an ta_uint32
                ta_uint32 number;
        ...
        {
            ta_uint32 trimLength = maxFieldSize - (textToSearchFor.size() + numberString.str().size());
    ...
    ProfileHelper::ActionGroupEntry::ActionGroupEntry(ta_uint32 actionGroupKey, bool isControl) :
    ...
    void ProfileHelper::initialiseMaxFieldLengths()
    {
        ...
        TA_THREADGUARD(m_nameMaxLengthLock);
        ...
        if (NULL == data || 0 == data->getNumRows())
        {
            ...
            LOG_ERROR("Could not retrieve column lengths. Setting to default values");
        ...
        else if (1 < data->getNumRows())
        {
            ...
            LOG_ERROR("More than expected entries retrieved while retrieving column lengths. Setting to default values");
    ...
    std::vector<ta_uint32> ProfileHelper::getLocationKeys()
    {
        ...
        std::vector<ta_uint32> locationKeys;
    ...
    void ProfileHelper::updateLocationAssociations()
    {
        ...
        std::vector<ta_uint32> locationKeys = getLocationKeys();
        ...
        std::vector<ta_uint32> itemsToDelete;
        std::vector<ta_uint32> itemsToInsert(m_associatedLocationKeys);
        ...
        if (uInsertSize > 0)
        {
            ...
            for (unsigned int i = 0; i < uInsertSize; i++)
            {
                ...
                case enumMysqlDb:
                    ...
                    break;
                default:
                    LOG_WARN("the type of the database server is not supported currently.");
                }*/
