 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/EquipReportHelper.cpp $
 * @author:  Ripple
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Core
{
    ...
    EquipReportHelper::EquipReportHelper(std::string& key, const std::string& name, const ta_uint32 locationId)
        : m_Key(key),
          m_Name(name),
          m_isValidData(true),
    ...
    EquipReportHelper::EquipReportHelper(const ta_uint32 row, TA_Base_Core::IData& data)
        :   m_isNew(false),
            m_isValidData(false)
    {
        FUNCTION_ENTRY("EquipReportHelper(const ta_uint32 , TA_Base_Core::IData&)");
    ...
    void EquipReportHelper::setLocationId(const ta_uint32 locationId)
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    void EquipReportHelper::setProfile(const ta_uint32 profile)
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    void EquipReportHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data)
    {
    ...
    void EquipReportHelper::addNewEquipReport()
    {
        ...
        // we can have names which are the same as ta_int32 as they are at different locations
    ...
    ta_uint32 EquipReportHelper::getLocationId()
    {
        // We don't want to call reload() if this is a new report and it has not been written to the database
        if (!m_isValidData && !m_isNew)
        {
    ...
    ta_uint32 EquipReportHelper::getProfile()
    {
        // We don't want to call reload() if this is a new report and it has not been written to the database
        if (!m_isNew)
        {
    ...
    ta_uint32 EquipReportHelper::getKey()
    {
        return 0;
    }
    ...
    std::string EquipReportHelper::getLocalDatabaseName()
    {
        if (m_localDatabase.empty())
        {
            ...
            try
            {
                ...
                m_localDatabase = DatabaseFactory::getInstance().getDatabaseName(strDbConnFile, Scada_Cd, Write);
