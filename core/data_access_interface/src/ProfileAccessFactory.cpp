 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/ProfileAccessFactory.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #4 $
 *
...
namespace TA_Base_Core
{
    ...
    IProfile* ProfileAccessFactory::getProfile(const ta_uint32 key, const bool readWrite)
    {
        // Get a specific profile
        FUNCTION_ENTRY("ProfileAccessFactory::getProfile");
    ...
    void ProfileAccessFactory::getProfileIDsBySql(IDatabase* pDatabase, const SQLStatement& sql, std::vector<ta_uint32>& profileIDs)
    {
        // Get a connection to the database
        TA_ASSERT(0 != pDatabase, "the database connection is null");
        ...
        do
        {
            ...
            for (ta_uint32 i = 0; i < numRows; i++)
            {
                ...
                try
                {
                    profileIDs.push_back(data->getUnsignedLongData(i, columnNames[0]));                    
                }
                //data exception
                catch (TA_Base_Core::DataException&)
                {
    ...
    // TD 10468
    ta_uint32 ProfileAccessFactory::getDeletedRawProfileType(ta_uint32 key)
    {
        FUNCTION_ENTRY("getDeletedRawProfileType");

        // Get a connection to the database
        IDatabase* databaseConnection =
            DatabaseFactory::getInstance().getDatabase({ OnlineSecurity_Cd, OnlineSecurity_Cached}, Read);
        ...
        if (0 == profiles.size())
        {
            ...
            std::string strExSql;
            auto nExDbType = databaseConnection->getDbServerType();

            switch (nExDbType)
            {
                case EDatabaseServerType::Oracle:
                    strExSql = (sql.strOracle.size() ? sql.strOracle : sql.strCommon);
                    break;

                case EDatabaseServerType::MySQL:
                    strExSql = (sql.strMySQL.size() ? sql.strMySQL : sql.strCommon);
                    break;

                case EDatabaseServerType::SQLite:
                    strExSql = (sql.strSQLite.size() ? sql.strSQLite : sql.strCommon);
                    break;

                case EDatabaseServerType::CacheDB:
                    strExSql = (sql.strSQLite.size() ? sql.strSQLite : sql.strCommon);
                    break;
            }
    ...
    std::string ProfileAccessFactory::getDeletedProfileName(ta_uint32 key)
    {
    ...
    ta_uint32 ProfileAccessFactory::getDeletedProfileGroup(ta_uint32 key)
    {
    ...
    std::string ProfileAccessFactory::getDeletedProfileGroupAsString(ta_uint32 key)
    {
        FUNCTION_ENTRY("getDeletedProfileGroup");
        ...
        IProfilePtr profile(profiles[0]);
        std::string profileGroup;

        try
        {
            profileGroup = profile->getAccessControlGroupAsString();
        }
        //exception
        catch (...)
        {
    ...
    bool ProfileAccessFactory::getDeletedProfileIsSystemProfile(ta_uint32 key)
    {
    ...
    std::vector<IProfile*> ProfileAccessFactory::getProfilesByOperator(ta_uint32 key, const bool readWrite)
    {
        ...
        SQLStatement sql;
        databaseConnection->prepareSQLStatement(sql, SE_OPERATOR_PROFILE_SELECT_5055, key);
        //      TA_ASSERT(sql.size() > 0, "getProfilesByOperator: Get SQL Statement error");        
        std::vector<ta_uint32> profileIDs;
        getProfileIDsBySql(databaseConnection, sql, profileIDs);

        std::vector<IProfile*> profiles;
        for (auto &profileID : profileIDs)
        {
            IProfile *iProfile = ProfileAccessFactoryEx::getProfile(profileID).get();
            
            if (readWrite)
            {
                if (auto configProfile = dynamic_cast<TA_Base_Core::ConfigProfile*>(iProfile); configProfile != NULL)
                {
                    profiles.push_back(new ConfigProfile(*configProfile));
                }
            }
            else
            {
                if (auto profile = dynamic_cast<TA_Base_Core::Profile*>(iProfile); profile != NULL)
                {
                    profiles.push_back(new Profile(profile->getKey()));
                }                 
            }
    ...
    std::vector<IProfile*> ProfileAccessFactory::getProfilesByOperator(const std::string& name, const bool readWrite)
    {
        ...
        SQLStatement sql;
        databaseConnection->prepareSQLStatement(sql, SE_OPERATOR_PROFILE_SELECT_5056,
    ...
    std::vector<IProfile*> ProfileAccessFactory::getProfilesByLocation(ta_uint32 locationId, const bool readWrite)
    {
    ...
    std::vector<IProfile*> ProfileAccessFactory::getNonSystemProfilesByOperator(ta_uint32 key, const bool readWrite)
    {
        ...
        for (auto &profileID : profileIDs)
        {
            ...
            if (readWrite)
            {
                if (ConfigProfile* configProfile = dynamic_cast<TA_Base_Core::ConfigProfile*>(iProfile);
                    configProfile != NULL)
                {
                    profiles.push_back(new ConfigProfile(*configProfile));
                }                
            ...
            {
                if (Profile* profile = dynamic_cast<TA_Base_Core::Profile*>(iProfile);
                    profile != NULL)
                {
                    profiles.push_back(new Profile(profile->getKey()));
                }                
    ...
    std::vector<IProfile*> ProfileAccessFactory::getNonSystemProfilesByOperator(const std::string& name, const bool readWrite)
    {
        ...
        SQLStatement sql;
        databaseConnection->prepareSQLStatement(sql, SE_OPERATOR_PROFILE_SELECT_5056, databaseConnection->escapeQueryString(name.c_str()));
