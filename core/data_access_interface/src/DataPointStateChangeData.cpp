 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/DataPointStateChangeData.cpp $
 * @author:  Nick Jardine
 * @version: $Revision: #3 $
 *
...
{
    DataPointStateChangeData::DataPointStateChangeData(const std::string& key, const std::map<std::string, ta_uint32>& alarmMessageDataPointNameToPKeyMap)
        : m_key(key), m_timestamp(0), m_logtime(0), m_entityKey(TA_MAX_UINT32), m_value(""), m_status(""), m_isValidData(false), m_isNewEntry(false),
          m_alarmMessageId(0), m_generalMessageId(-1), m_alarmMessageDataPointNameToPKeyMap(alarmMessageDataPointNameToPKeyMap), m_eventTypeKey(TA_MAX_UINT32)
    {
    }

    DataPointStateChangeData::DataPointStateChangeData(const std::map<std::string, ta_uint32>& alarmMessageDataPointNameToPKeyMap)
        : m_key(""), m_timestamp(0), m_logtime(0),
          m_entityKey(TA_MAX_UINT32), m_value(""), m_status(""), m_isValidData(true), m_isNewEntry(true),
          m_alarmMessageId(0), m_generalMessageId(-1), m_alarmMessageDataPointNameToPKeyMap(alarmMessageDataPointNameToPKeyMap), m_eventTypeKey(TA_MAX_UINT32)
    {
    ...
    DataPointStateChangeData::DataPointStateChangeData(const std::string& key, const time_t timeStamp,
                                                       const time_t logTime, const ta_uint32 entityKey,
                                                       ...
                                                       const std::map<std::string, ta_uint32>& alarmMessageDataPointNameToPKeyMap)
        : m_key(key), m_timestamp(timeStamp), m_logtime(logTime), m_entityKey(entityKey), m_value(value), m_status(status),
          ...
          m_alarmMessageDataPointNameToPKeyMap(alarmMessageDataPointNameToPKeyMap), m_eventTypeKey(TA_MAX_UINT32)
    {
    }
    ...
    const ta_uint32 DataPointStateChangeData::getEntityKey()
    {
        if (m_isValidData)
        {
            TA_ASSERT(TA_MAX_UINT32 != m_entityKey, "The entity key must be set before it can be read");
        }
        else
        {
            reload();
    ...
    void DataPointStateChangeData::setEntityKey(const ta_uint32 entityKey)
    {
        if (!m_isValidData)
        {
            reload();
    ...
    // Raymond Pau++
    // Bug#802 TD7194
    void DataPointStateChangeData::setMessage(ta_int32 generalMessage)
    {
    ...
    void DataPointStateChangeData::setMessage(const std::string& messageType)
    {
        ...
        std::map<std::string, ta_uint32>::const_iterator it =
        ...
        {
            LOG_ERROR("Unknown messageType - %s", messageType);
        }

        FUNCTION_EXIT;
    }

    const ta_uint32 DataPointStateChangeData::getEventTypeKey()
    {
        if (m_isValidData)
        {
            TA_ASSERT(TA_MAX_UINT32 != m_eventTypeKey, "The entity key must be set before it can be read");
        }
        else
        {
            reload();
    ...
    void DataPointStateChangeData::setEventTypeKey(const ta_uint32 eventTypeKey)
    {
        if (!m_isValidData)
        {
            reload();
    ...
    std::string DataPointStateChangeData::getLocalDatabaseName()
    {
        if (m_localDatabase.empty())
        {
            ...
            try
            {
                ...
                m_localDatabase = DatabaseFactory::getInstance().getDatabaseName(strDbConnFile, Event_Ad, Write);
    ...
    void DataPointStateChangeData::reload()
    {
        ...
        IDatabase* databaseConnection =
            DatabaseFactory::getInstance().getDatabase(Event_Ad, Read);
    ...
    void DataPointStateChangeData::applyChanges()
    {
        ...
        bool entityKeyValid = TA_MAX_UINT32 != m_entityKey;
        ...
        if (0 < fieldNames.size())
        {
            ...
            for (ta_uint32 i = 0; i < fieldNames.size(); i++)
            {
                fields += fieldNames[i] + ", ";
            }
