 ...
 * in any form.
 *
 * Source: $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/IDatabase.h $
 * @author Nick Jardine
 * @version $Revision: #3 $
...
namespace TA_Base_Core
{
    ...
    public:
         ...
         * getSqlString
         *
         * This method get the real SQL string for the connected database.
         *  or return empty string if invalid.
         *
         */
        virtual std::string getSqlString(const SQLStatement& sqlStatement) = 0;
        ...
        virtual IData* executeQuery(const SQLStatement& rSqlObj, const std::vector<std::string>& columnNames, size_t numRows, const std::string& options = "") = 0;
        virtual IData* executeQuery(const SQLStatement& rSqlObj, const std::vector<std::string>& columnNames, const std::string& options = "") = 0;

        virtual IDataPtr executeQueryAll(const SQLStatement& rSqlObj, const std::vector<std::string>& columnNames)
        {
            return IDataPtr(executeQuery(rSqlObj, columnNames, INT_MAX, DATABASE_QUERY_OPTION_RELAX));
        }

        virtual IDataPtr executeQueryAll(const SQLStatement& rSqlObj)
        {
            return IDataPtr(executeQuery(rSqlObj, {}, INT_MAX, DATABASE_QUERY_OPTION_RELAX " " DATABASE_QUERY_OPTION_WITH_COLUMN_NAMES));
        }
         ...
         *      connect() MUST be called prior to this method
         */
        virtual IData* executeProcedure(const SQLStatement& rSqlObj, const std::vector<std::string>& columnNames, const ta_uint32 numRows = 500) = 0;
         ...
         *
         */
        virtual void readBLOB(const std::string& fieldName, const std::string& tableName, const std::string& whereClause, std::vector<unsigned char>& data) = 0;
         ...
         *
         */
        virtual void writeBLOB(const std::string& fieldName, const std::string& tableName, const std::string& whereClause, std::vector<unsigned char>& data) = 0;
		 ...
		 * getNextPkeyDynamic
		 *
		 * get next pkey for insert msyql script
		 * @return int
		 * @param : const std::string& str, it is sequence name
		 *
		 * @exception <exceptions> Optional
		 */
		virtual std::string getNextPkeyDynamic(const std::string& strSeqName) = 0;
         ...
         *
         */
        virtual EDatabaseServerType getDbServerType() = 0;
        ...
        virtual void prepareSQLStatement(SQLStatement& rSqlStatement, const SQLVarParms& varParms) = 0;

        template <typename... Args>
        void prepareSQLStatement(SQLStatement& rSqlStatement, Args&& ... args)
        {
            SQLVarParms params;
            int unroll[] = { (params << std::forward<Args>(args), 0)..., 0 };
            prepareSQLStatement(rSqlStatement, const_cast<const SQLVarParms&>(params));
        }

        template <typename... Args>
        SQLStatement preparedSQLStatement(Args&& ... args)
        {
            SQLStatement stmt;
            prepareSQLStatement(stmt, std::forward<Args>(args)...);
            return stmt;
        }
