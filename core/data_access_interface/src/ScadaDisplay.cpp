 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/ScadaDisplay.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #3 $
 *
...
{
    ScadaDisplay::ScadaDisplay(const ta_uint32 key)
        : m_displayHelper(new ScadaDisplayHelper(key))
    {
    }

    ScadaDisplay::ScadaDisplay(const ta_uint32 row, TA_Base_Core::IData& data)
    ...
    ta_uint32 ScadaDisplay::getLocation()
    {
        TA_ASSERT(m_displayHelper != NULL, "The ScadaDisplayHelper pointer is null.");
