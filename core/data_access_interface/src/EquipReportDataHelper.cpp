 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/EquipReportDataHelper.cpp $
 * @author:  Ripple
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Core
{
    ...
    EquipReportDataHelper::EquipReportDataHelper(const ta_uint32 row, TA_Base_Core::IData& data)
        :
        m_Key(0),
        m_isNew(false),
        m_isValidData(false)
    {
        FUNCTION_ENTRY("EquipReportDataHelper(const ta_uint32 , TA_Base_Core::IData&)");
    ...
    void EquipReportDataHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data)
    {
    ...
    void EquipReportDataHelper::setDataNodeKey(const ta_uint32 aDataNodeKey)
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    void EquipReportDataHelper::setDataPointKey(const ta_uint32 aDataPointKey)
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    ta_uint32 EquipReportDataHelper::getDataNodeKey()
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    ta_uint32 EquipReportDataHelper::getDataPointKey()
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    ta_uint32 EquipReportDataHelper::getKey()
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
