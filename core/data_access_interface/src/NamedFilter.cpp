 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/NamedFilter.cpp $
 * @author:  Bradley Cleaver
 * @version: $Revision: #5 $
 *
 * Last modification: $DateTime: 2025/09/04 16:36:17 $
 * Last modified by:  $Author: youming.liu $
 *
...
{
    const ta_uint32 NamedFilter::LIST_TYPE_STRING(0);
    const ta_uint32 NamedFilter::LIST_TYPE_NUMERIC(1);
    const std::string NamedFilter::UNNAMED("用戶自定義");
    ...
    void NamedFilter::addNumericFilter(const std::string& listName, const ta_uint32 value)
    {
        TA_THREADGUARD(m_filterLock);
        ...
        if (it != m_numericFilters.end())
        {
            ...
            NumericList::iterator insertPt = std::find_if(it->second.begin(), it->second.end(),
                                                          std::bind2nd(std::greater<ta_uint32>(), value));
    ...
    ta_uint32 NamedFilter::getApplicationKey()
    {
        // Convert the type to a number using a string stream.
        ta_uint32 applicationKey;
    ...
    void NamedFilter::saveFilterItem(const std::string& listName, const ta_uint32 listType, const std::string& listItem)
    {
        // Get a connection to the database.
        IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase(AlarmsLocal_Cd, Write);
