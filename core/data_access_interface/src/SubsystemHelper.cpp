 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/SubsystemHelper.cpp $
...
namespace TA_Base_Core
{
    ...
    SubsystemHelper::SubsystemHelper(const ta_uint32 key)
        :
        m_key(key),
        m_name(""),
        m_isExclusive(false),
    ...
    SubsystemHelper::SubsystemHelper(const ta_uint32 row, TA_Base_Core::IData& data)
        :
        m_isNew(false),
        m_isValidData(false)
    {
        FUNCTION_ENTRY("SubsystemHelper(const ta_uint32 , TA_Base_Core::IData&)");
    ...
    ta_uint32 SubsystemHelper::getKey()
    {
        TA_ASSERT(! m_isNew,
    ...
    ta_uint32 SubsystemHelper::getSystemKey()
    {
        TA_ASSERT(! m_isNew,
                  "Cannot retrieve the key on a subsystem before it "
                  "has been applied to the database");
	...
	void SubsystemHelper::setPhysicalSubsystemStatus(bool isPhysical)
	{
		// We don't want to call reload() if this is a new subsystem
		// and it has not been written to the database
		if (!m_isValidData && !m_isNew)
    ...
    void SubsystemHelper::modifyExistingSubsystem()
    {
        ...
        LOG_DEBUG("Subsystem %s already exists. Its details will be updated.", m_name);

        // Get a connection to the database
        IDatabase* databaseConnection =
            DatabaseFactory::getInstance().getDatabase(OnlineUpdatable_Cd, Read);
        ...
        databaseConnection->prepareSQLStatement(strSql, SUBSYSTEM_UPDATE_18151,
                                                databaseConnection->escapeQueryString(m_name), (databaseConnection->escapeQueryString(m_displayName)), m_isExclusive, m_isLocationExclusive, m_key);
    ...
    void SubsystemHelper::addNewSubsystem()
    {
        ...
        LOG_DEBUG("Subsystem %s is a new subsystem. " "It will be added to the database.", m_name);

        // get a connection to the database
        IDatabase* databaseConnection =
            DatabaseFactory::getInstance().getDatabase(OnlineUpdatable_Cd/*OnlineUpdatable_Cd*/, Write);
        ...
        databaseConnection->prepareSQLStatement(strSql, SUBSYSTEM_INSERT_18351,
                                                ...
                                                databaseConnection->escapeInsertString(m_name), 
			                                    databaseConnection->escapeInsertString(m_displayName), 
			                                    m_isPhysical, m_isExclusive, m_isLocationExclusive);
        ...
        // Retrieve the pkey as an ta_uint32 from the
    ...
    void SubsystemHelper::checkForReferencesFromEntityTable()
    {
        ...
        if (0 != data->getNumRows())
        {
            try
            {
                ...
                TA_THROW(DataException(message.c_str(), DataException::CANNOT_DELETE, entities));
            }
            //exception
            catch (...)
            {
    ...
    void SubsystemHelper::checkForReferencesFromEntityTypeTable()
    {
        ...
        if (0 != data->getNumRows())
        {
            try
            {
                ...
                TA_THROW(DataException(message.c_str(), DataException::CANNOT_DELETE, types));
            }
            //exception
            catch (...)
            {
    ...
    void SubsystemHelper::getColumnNames(std::vector<std::string>& columnNames)
    {
        ...
        columnNames.push_back("LOC_EXCLUSIVE_CONTROL");
    ...
    void SubsystemHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data)
    {
        ...
        try
        {
            ...
            m_isLocationExclusive   = data.getBooleanData(row, columnNames[5]);
            m_dateModified = data.getDateData(row, columnNames[6], 0);
            m_dateCreated   = data.getDateData(row, columnNames[7]);
            m_systemKey = data.getUnsignedLongData(row, columnNames[8]);
        ...
        {
            LOG_EXCEPTION("DataException", "Could not apply the item changes");
            ...
            LOG_ERROR("Error happen when get data for subsystem, error detail: %s", reason);
        }
        //exception
        catch (...)
        {
            LOG_ERROR("Error happen when get data for subsystem, unknown error.");
        }
