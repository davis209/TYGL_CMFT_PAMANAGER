 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/MmsRequestAccessFactory.cpp $
 * @author:  Luo HuiRong
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Core
{
    ...
    //  {
    // /*       char pKeyString[50];*/
    //       ta_uint32   severityKey=MmsRequest.getAlarmSeverity();
    ...
    //  {
    //       ta_uint32 entityKey=MmsRequest.getEntityKey();
    //
    ...
    //      std::string entityKeyStr = DataConversion::toString(entityKey);
    //
    //       ta_uint32 scheduledType = static_cast<ta_uint32>(MmsRequest.getScheduledType());
    //
    ...
    std::string  MmsRequestAccessFactory::getDeleteSQL(IDatabase* databaseConnection, std::vector<MmsRequestAlarmData*>& MmsRequests)
    {
        ...
        for (int i = 0; i < MmsRequests.size(); i++)
        {
            ...
            {
                ta_uint32 pkey = MmsRequests[i]->getKey();
	...
	std::string MmsRequestAccessFactory::getDeleteSQL(IDatabase* databaseConnection, std::vector<std::string>& keylist)
	{
		std::string formatSQL = "";
		//formatSQL += "delete mms_request_scheduled where pkey in (";
		int pkeyCount = 0;
    ...
    {
        LOG_TRACE("MmsRequestAccessFactory::saveMmsRequestAlarm in");
        // Now we can get on with writing the entity to the database.
        // Convert non-string elements to char*'s

        // Get the database connection
        IDatabase* databaseConnection =
            DatabaseFactory::getInstance().getDatabase(MMSRequest, Write);
        // generate the SQL string to write the entity (reuse the string)

        ta_uint32   severityKey = MmsRequest.getAlarmSeverity();
        std::string severityKeyStr = DataConversion::toString(severityKey);

		ta_uint32 alarmState = MmsRequest.getAlarmState();
		std::string alarmStateStr = DataConversion::toString(alarmState);
        ...
        databaseConnection->prepareSQLStatement(strSql, MMS_REQUEST_ALARM_INSERT_73001, getLocalDatabaseName(), databaseConnection->escapeAQSQLString(MmsRequest.getAlarmID()),
                                                ...
                                                databaseConnection->escapeAQSQLString(MmsRequest.getRequestor()), alarmStateStr, isAutoTriggerString);
        ...
        DatabaseFactory::getInstance().delDatabase(databaseConnection);

        LOG_TRACE("MmsRequestAccessFactory::saveMmsRequestAlarm out");
        return true;
    }
    ...
    {
        LOG_TRACE("MmsRequestAccessFactory::saveMmsRequestSchematic in");
        ...
        databaseConnection->prepareSQLStatement(strSql, MMS_REQUEST_SCHEMATIC_INSERT_73101, getLocalDatabaseName(), entityKeyStr,
                                                ...
                                                databaseConnection->escapeAQSQLString(MmsRequest.getProblemReport()), std::string(targetTimeString), std::string(createDateString));

        //      TA_ASSERT(strSql.size() > 0, "MmsRequestAccessFactory::saveMmsRequestSchematic(): Prepare SQL Statement error");
        databaseConnection->executeModification(strSql);

        LOG_TRACE("MmsRequestAccessFactory::saveMmsRequestSchematic out");
        return true;
    }

    bool MmsRequestAccessFactory::saveMmsRequestScheduled(MmsRequestScheduledData& MmsRequest)
    {
        LOG_TRACE("MmsRequestAccessFactory::MmsRequestScheduledData in");
        ...
        LOG_TRACE("MmsRequestAccessFactory::MmsRequestScheduledData out");
        return true;
    }

    std::vector<MmsRequestAlarmData*> MmsRequestAccessFactory::getMmsRequestAlarmData(ta_uint32 maxSize)
    {
        //order by alarm severity (1 highest first), alarm date time (latest first)
        LOG_TRACE("MmsRequestAccessFactory::getMmsRequestAlarmData in");

        // Get a connection to the database
        IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase(MMSRequest, Read);
		...
		columnNames.push_back("ALARMSTATE");
        ...
        LOG_TRACE("MmsRequestAccessFactory::getMmsRequestAlarmData out");
        return mmsRequestAlarms;
    }

	std::vector<MmsRequestAlarmData*> MmsRequestAccessFactory::getMmsAlarmData(time_t latestAlarm, ta_uint32 maxSize)
	{
		//order by alarm severity (1 highest first), alarm date time (latest first)
		LOG_TRACE("MmsRequestAccessFactory::getMmsRequestAlarmData in");
		...
		if (ACE_OS::localtime_r(&latestAlarm, &sourceTimeStruct) != NULL)
		{
			sprintf(sourceTimeString, "%02d/%02d/%04d %02d:%02d:%02d", sourceTimeStruct.tm_mday, sourceTimeStruct.tm_mon + 1,
				sourceTimeStruct.tm_year + 1900, sourceTimeStruct.tm_hour, sourceTimeStruct.tm_min, sourceTimeStruct.tm_sec);
		}
		...
		//      std::string sql  = databaseConnection->prepareSQLStatement(MMS_REQUEST_ALARM_SELECT_73002,maxSize);
		SQLStatement sql;
		databaseConnection->prepareSQLStatement(sql, MMS_REQUEST_ALARM_SELECT_73004, std::string(sourceTimeString), maxSize);
		//      TA_ASSERT(sql.size() > 0, "MmsRequestAccessFactory::getMmsRequestAlarmData(): Prepare SQL Statement error");
	...
	std::vector<MmsRequestAlarmData*> MmsRequestAccessFactory::getMmsJobRequestData(time_t latestJobRequest, ta_uint32 maxSize)
	{
		//order by alarm severity (1 highest first), alarm date time (latest first)
		LOG_TRACE("MmsRequestAccessFactory::getMmsRequestAlarmData in");
		...
		if (ACE_OS::localtime_r(&latestJobRequest, &sourceTimeStruct) != NULL)
		{
			sprintf(sourceTimeString, "%02d/%02d/%04d %02d:%02d:%02d", sourceTimeStruct.tm_mday, sourceTimeStruct.tm_mon + 1,
				sourceTimeStruct.tm_year + 1900, sourceTimeStruct.tm_hour, sourceTimeStruct.tm_min, sourceTimeStruct.tm_sec);
		}
		...
		//      std::string sql  = databaseConnection->prepareSQLStatement(MMS_REQUEST_ALARM_SELECT_73002,maxSize);
		SQLStatement sql;
		databaseConnection->prepareSQLStatement(sql, MMS_REQUEST_ALARM_SELECT_73005, std::string(sourceTimeString), maxSize);
		//      TA_ASSERT(sql.size() > 0, "MmsRequestAccessFactory::getMmsRequestAlarmData(): Prepare SQL Statement error");
    ...
    MmsRequestAlarmData* MmsRequestAccessFactory::createMmsRequestAlarmData(ta_uint32 row, IData* data)
    {
        ta_uint32 pkey = data->getUnsignedLongData(row, "PKEY");
        ...
        ta_uint32 alarmSeverity = data->getUnsignedLongData(row, "ALARMSEVERITY");
		ta_uint32 alarmState = data->getUnsignedLongData(row, "ALARMSTATE");
        ...
        return new MmsRequestAlarmData(pkey, alarmID, alarmDate_timeb, alarmSeverity, equipmentID, alarmFaultCode,
                                       alarmDescription, requestor, problemReport, targetDate_timeb, isAutoTrigger, alarmState);
    }

    MmsRequestSchematicData* MmsRequestAccessFactory::createMmsRequestSchematicData(ta_uint32 row, IData* data)
    {
        ta_uint32 key = data->getUnsignedLongData(row, "PKEY");
        ta_uint32 entityKey = data->getUnsignedLongData(row, "ENTITYKEY");
        std::string equipmentID = data->getStringData(row, "EQUIPMENTID");
        std::string requestor = data->getStringData(row, "REQUESTOR");
        std::string problemReport = data->getStringData(row, "PROBLEMREPORT");
        ...
        return new MmsRequestSchematicData(key, entityKey, equipmentID, requestor, problemReport, targetDate_timeb, createDate_timeb);
    }

    MmsRequestScheduledData* MmsRequestAccessFactory::createMmsRequestScheduledData(ta_uint32 row, IData* data)
    {
        ta_uint32 key = data->getUnsignedLongData(row, "PKEY");
    ...
    std::vector<MmsRequestSchematicData*> MmsRequestAccessFactory::getMmsRequestSchematicData(ta_uint32 maxSize)
    {
        //order by TARGETDATE (earliest first)
        LOG_TRACE("MmsRequestAccessFactory::getMmsRequestSchematicData in");
        ...
        columnNames.push_back("PROBLEMREPORT");
        columnNames.push_back("TARGETDATE");
		columnNames.push_back("CREATETDATE");
        ...
        LOG_TRACE("MmsRequestAccessFactory::getMmsRequestSchematicData out");
        return mmsRequestAlarms;
    }

	std::vector<MmsRequestSchematicData*> MmsRequestAccessFactory::getMmsSchematicData(time_t lastSchematic, ta_uint32 maxSize)
	{
		//order by TARGETDATE (earliest first)
		LOG_TRACE("MmsRequestAccessFactory::getMmsRequestSchematicData in");
		...
		if (ACE_OS::localtime_r(&lastSchematic, &sourceTimeStruct) != NULL)
		{
			sprintf(sourceTimeString, "%02d/%02d/%04d %02d:%02d:%02d", sourceTimeStruct.tm_mday, sourceTimeStruct.tm_mon + 1,
				sourceTimeStruct.tm_year + 1900, sourceTimeStruct.tm_hour, sourceTimeStruct.tm_min, sourceTimeStruct.tm_sec);
		}
		...
		//      std::string sql  = databaseConnection->prepareSQLStatement(MMS_REQUEST_SCHEMATIC_SELECT_73102,maxSize);
		SQLStatement sql;
		databaseConnection->prepareSQLStatement(sql, MMS_REQUEST_SCHEMATIC_SELECT_73104, std::string(sourceTimeString), maxSize);
		//      TA_ASSERT(sql.size() > 0, "MmsRequestAccessFactory::getMmsRequestSchematicData(): Prepare SQL Statement error");
    ...
    std::vector<MmsRequestScheduledData*> MmsRequestAccessFactory::getMmsRequestScheduledData(ta_uint32 maxSize)
    {
        //order by PlanDate desc (latest first)
        LOG_TRACE("MmsRequestAccessFactory::getMmsRequestScheduledData in");
        ...
        LOG_TRACE("MmsRequestAccessFactory::getMmsRequestScheduledData out");
        return mmsRequestAlarms;
    }

	std::vector<MmsRequestScheduledData*> MmsRequestAccessFactory::getMmsCounterData(time_t latestCounter, ta_uint32 maxSize)
	{
		//order by PlanDate desc (latest first)
		LOG_TRACE("MmsRequestAccessFactory::getMmsRequestScheduledData in");
		...
		if (ACE_OS::localtime_r(&latestCounter, &sourceTimeStruct) != NULL)
		{
			sprintf(sourceTimeString, "%02d/%02d/%04d %02d:%02d:%02d", sourceTimeStruct.tm_mday, sourceTimeStruct.tm_mon + 1,
				sourceTimeStruct.tm_year + 1900, sourceTimeStruct.tm_hour, sourceTimeStruct.tm_min, sourceTimeStruct.tm_sec);
		}
		...
		//      std::string sql  = databaseConnection->prepareSQLStatement(MMS_REQUEST_SCHEDULED_SELECT_73202,maxSize);
		SQLStatement sql;
		databaseConnection->prepareSQLStatement(sql, MMS_REQUEST_SCHEDULED_SELECT_73204, std::string(sourceTimeString),  maxSize);
		//      TA_ASSERT(sql.size() > 0, "MmsRequestAccessFactory::getMmsRequestScheduledData(): Prepare SQL Statement error");
	...
	std::vector<MmsRequestScheduledData*> MmsRequestAccessFactory::getMmsMeasurementData(time_t latestMeasurment, ta_uint32 maxSize)
	{
		//order by PlanDate desc (latest first)
		LOG_TRACE("MmsRequestAccessFactory::getMmsRequestScheduledData in");
		...
		if (ACE_OS::localtime_r(&latestMeasurment, &sourceTimeStruct) != NULL)
		{
			sprintf(sourceTimeString, "%02d/%02d/%04d %02d:%02d:%02d", sourceTimeStruct.tm_mday, sourceTimeStruct.tm_mon + 1,
				sourceTimeStruct.tm_year + 1900, sourceTimeStruct.tm_hour, sourceTimeStruct.tm_min, sourceTimeStruct.tm_sec);
		}
		...
		//      std::string sql  = databaseConnection->prepareSQLStatement(MMS_REQUEST_SCHEDULED_SELECT_73202,maxSize);
		SQLStatement sql;
		databaseConnection->prepareSQLStatement(sql, MMS_REQUEST_SCHEDULED_SELECT_73205, std::string(sourceTimeString), maxSize);
		//      TA_ASSERT(sql.size() > 0, "MmsRequestAccessFactory::getMmsRequestScheduledData(): Prepare SQL Statement error");
    ...
    bool MmsRequestAccessFactory::deleteMmsRequestAlarmData(std::vector<MmsRequestAlarmData*>& vec)
    {
        ...
        IDatabase* databaseConnection =
            DatabaseFactory::getInstance().getDatabase(MMSRequest, Read); //only need to delete on Central Database
        ...
        if ("" != deleteSQL)
        {
			...
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "%s", deleteSQL.c_str());
	...
	bool MmsRequestAccessFactory::deleteMmsRequestAlarmData(std::vector<std::string>& vec)
	{
		if (vec.size() == 0)
		{
			return true;
	...
	bool MmsRequestAccessFactory::deleteMmsRequestSchematicData(std::vector<std::string>& vec)
	{
		if (vec.size() == 0)
		{
			return true;
	...
	bool MmsRequestAccessFactory::deleteMmsRequestScheduledData(std::vector<std::string>& vec)
	{
		if (vec.size() == 0)
		{
			return true;
