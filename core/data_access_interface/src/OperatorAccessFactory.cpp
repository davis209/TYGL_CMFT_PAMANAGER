 ...
 * in any form.
 *
 * Source: $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/OperatorAccessFactory.cpp $
 * @author Nick Jardine
 * @version $Revision: #3 $
...
namespace TA_Base_Core
{
    ...
    */
    ...
    {
        if (m_operatorToProfileMap.empty() || m_operatorToRegionMap.empty())
        {
            auto f1 = boost::async([&] { initializeOperatorProfile();});
            auto f2 = boost::async([&] { initializeOperatorRegion();});
    ...
    void OperatorAccessFactory::initializeOperatorProfile()
    {
        ...
        std::call_once(m_operatorToProfileLoad, [&]
        {
            // get a connection to the database
            IDatabase* database = DatabaseFactory::getInstance().getDatabase({OnlineSecurity_Cd, OnlineSecurity_Cached}, Read);
            ...
            auto sql = database->preparedSQLStatement(SE_OPERATOR_PROFILE_SELECT_5001);
            ...
            {
                auto operatorId = data->getUnsignedLongData(i, "OPERATORKEY");
                auto profileId  = data->getUnsignedLongData(i, "SEPROF_ID");
    ...
    void OperatorAccessFactory::initializeOperatorRegion()
    {
        ...
        std::call_once(m_operatorToRegionLoad, [&]
        {
            // get a connection to the database
            IDatabase* database = DatabaseFactory::getInstance().getDatabase({OnlineSecurity_Cd, OnlineSecurity_Cached}, Read);
            ...
            auto sql = database->preparedSQLStatement(SE_OPERATOR_REGIONS_SELECT_5501);
            ...
            {
                auto operatorId = data->getUnsignedLongData(i, "OPERATORKEY");
                auto regionId  = data->getUnsignedLongData(i, "SEREGI_ID");
                m_operatorToRegionMap[operatorId].push_back(regionId);
            }
        });
    }

    void OperatorAccessFactory::getProfileByOperator(ta_uint32 operatorId, std::vector<ta_uint32>& profiles)
    {
        FUNCTION_ENTRY("OperatorAccessFactory::getProfileByOperator");
        profiles = getProfileByOperator(operatorId);
        FUNCTION_EXIT;
    }

    void OperatorAccessFactory::getRegionByOperator(ta_uint32 operatorId, std::vector<ta_uint32>& regions)
    {
        FUNCTION_ENTRY("OperatorAccessFactory::getRegionByOperator");
        regions = getRegionByOperator(operatorId);
        FUNCTION_EXIT;
    }

    std::vector<ta_uint32>& OperatorAccessFactory::getProfileByOperator(ta_uint32 operatorId)
    {
        init();
        static std::vector<ta_uint32> s_empty;
        return stdutil::get_value_or(m_operatorToProfileMap, operatorId, s_empty);
    }

    std::vector<ta_uint32>& OperatorAccessFactory::getRegionByOperator(ta_uint32 operatorId)
    {
        init();
        static std::vector<ta_uint32> s_empty;
        return stdutil::get_value_or(m_operatorToRegionMap, operatorId, s_empty);
    }
        ...
        const bool getRegions)
    {
        ...
        //columnNames.push_back("S_VAL"); // [aqiu]: random SALT value should not be part of IOperator, as it is considered implementation specific.
        ...
        // Bohong++
        std::vector<ta_uint32> operatorIds;
        ...
        do
        {
            ...
            {
                ta_uint32 operatorId = data->getUnsignedLongData(i, columnNames[0]);
        ...
        {
            for (ta_uint32 i = 0; i < (*it)->getNumRows(); ++i)
            {
        ...
        for (auto row : *data)
        {
            auto id = row.getUnsignedLongData(columnNames[0]);

            auto& profiles = getProfileByOperator(id);
            auto& regions = getRegionByOperator(id);

            if (readWrite)
            {
                operators.push_back(new ConfigOperator(row.m_index, row.m_data, profiles, regions));
            }
            else
            {
                operators.push_back(new Operator(row.m_index, row.m_data, profiles, regions));
            }
        }
    ...
    IOperator* OperatorAccessFactory::getOperator(const ta_uint32 key, const bool readWrite)
    {
        FUNCTION_ENTRY("getOperator");

        // get a connection to the database
        IDatabase* databaseConnection =
            DatabaseFactory::getInstance().getDatabase({OnlineUpdatable_Cd, OnlineUpdatable_Cached}, Read);
    ...
    bool OperatorAccessFactory::getOperatorSaltValue(const ta_uint32 key, std::string& saltValue)
    {

        FUNCTION_ENTRY("getOperatorSaltValue");
        ...
        SQLStatement sql;
        // select S_VAL from operator_v where PKEY = %s
        databaseConnection->prepareSQLStatement(sql, OPERATOR_SELECT_6005, key);
        //      TA_ASSERT(sql.size() > 0, "getOperator: Get SQL Statement error");

        std::vector<std::string> columnNames;
        columnNames.push_back("S_VAL");

        IData* data = databaseConnection->executeQuery(sql, columnNames);

        if(1 != data->getNumRows())
        {
            LOG_ERROR("Error reading Salt Value for operator key = ", key);
            return false;
        }

        saltValue = data->getStringData(0, columnNames[0]);
    ...
    //TD15069++
    std::string OperatorAccessFactory::getDeletedOperatorName(const ta_uint32 key)
    {
    ...
    //TD15069++
    bool OperatorAccessFactory::getDeletedOperatorIsSystemOperator(const ta_uint32 key)
    {
