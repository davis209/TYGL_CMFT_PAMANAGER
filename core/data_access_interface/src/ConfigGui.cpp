 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/ConfigGui.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Core
{
    ...
    const std::string ConfigGui::MAX_INSTANCES = "Max Instances";
    ...
    const ta_uint32 ConfigGui::MAXIMUM_RESERVED_KEY = 99;

    const int ConfigGui::MAX_ENTITY_NAME_LENGTH = 80;
    const int ConfigGui::MAX_EXECUTABLE_LENGTH  = 1024;

    ta_int32 ConfigGui::s_nextAvailableIdentifier = 0;
          ...
          m_noType(type.empty())
    {
        if (!m_noType)
        {
            ...
            {
                LOG_EXCEPTION("Unknown", "Could not retrieve a Control Station entity to use as the parent of our new Gui entity");
            }
        }
    }

    ConfigGui::ConfigGui(const ta_uint32 key)
    ...
    ConfigGui::ConfigGui(const ta_uint32 row, TA_Base_Core::IData& data)
        :
    ...
    ta_uint32 ConfigGui::getAppMaxInstances()
    {
        TA_ASSERT(m_guiHelper != NULL, "The ConfigGuiHelper pointer is null.");
    ...
    void ConfigGui::setAppMaxInstances(const std::string& numInstances)
    {
        TA_ASSERT(m_guiHelper != NULL, "The ConfigGuiHelper pointer is null.");
        
        std::string newValue = numInstances;
        std::string oldValue = std::to_string(m_guiHelper->getAppMaxInstances());

        m_guiHelper->setAppMaxInstances(newValue);
        updateGuiChanges(MAX_INSTANCES, oldValue, newValue);
    }
