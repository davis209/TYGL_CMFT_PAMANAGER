 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/OnlinePrinterHelper.cpp $
 * @author:  Nick Jardine
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Core
{
    ...
    OnlinePrinterHelper::OnlinePrinterHelper(const ta_uint32 idKey)
        : m_idKey(idKey),
          m_printer(""),
          m_location(0),
          m_subsystem(0),
    ...
    OnlinePrinterHelper::OnlinePrinterHelper(ta_uint32 row, TA_Base_Core::IData& data)
        : m_idKey(-1),
          m_printer(""),
          m_location(0),
          m_subsystem(0),
    ...
    ta_uint32 OnlinePrinterHelper::getKey()
    {
    ...
    ta_uint32 OnlinePrinterHelper::getLocation()
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    ta_uint32 OnlinePrinterHelper::getSubsystem()
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    void OnlinePrinterHelper::setLocation(ta_uint32 pLocation)
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    void OnlinePrinterHelper::setSubsystem(ta_uint32 pSubsystem)
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    void OnlinePrinterHelper::reload()
    {
        ...
        TA_Base_Core::IDatabase* databaseConnection =
            DatabaseFactory::getInstance().getDatabase(TA_Base_Core::Ats_SCd/*Ats_Cd*/, TA_Base_Core::Read);
    ...
    void OnlinePrinterHelper::reloadUsing(ta_uint32 row, TA_Base_Core::IData& data)
    {
    ...
    void OnlinePrinterHelper::writeOnlinePrinterData()
    {
    ...
    void OnlinePrinterHelper::deleteOnlinePrinter(bool cascade)
    {
        ...
        TA_Base_Core::IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase(TA_Base_Core::Ats_SCd/*Ats_Cd*/, TA_Base_Core::Read);
        /*std::ostringstream sql;
    ...
    void OnlinePrinterHelper::modifyExistingOnlinePrinter()
    {
        FUNCTION_ENTRY("modifyExistingOnlinePrinter");

        LOG_DEBUG("OnlinePrinter id=%ul already exists. It's data will be updated.", m_idKey);

        // get a connection to the database
        TA_Base_Core::IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase(TA_Base_Core::Ats_SCd/*Ats_Cd*/, TA_Base_Core::Write);
        /*std::ostringstream sql;
    ...
    void OnlinePrinterHelper::addNewOnlinePrinter()
    {
        ...
        LOG_DEBUG("OnlinePrinter %ul is a new OnlinePrinter. It will be added to the database.", m_idKey);
