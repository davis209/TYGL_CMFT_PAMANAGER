 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/MmsRequestAccessFactory.h $
 * @author:  Luo HuiRong
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Core
{
    ...
    public:
         ...
         *            3) Error while attempting to execute a database query
         */
        std::vector<MmsRequestAlarmData*> getMmsRequestAlarmData(ta_uint32 maxSize = 10000);
        std::vector<MmsRequestSchematicData*> getMmsRequestSchematicData(ta_uint32 maxSize = 10000);
        std::vector<MmsRequestScheduledData*> getMmsRequestScheduledData(ta_uint32 maxSize = 10000);

		std::vector<MmsRequestAlarmData*> getMmsAlarmData(time_t latestAlarm, ta_uint32 maxSize = 10);
		std::vector<MmsRequestAlarmData*> getMmsJobRequestData(time_t latestJobRequest, ta_uint32 maxSize = 10);
		std::vector<MmsRequestSchematicData*> getMmsSchematicData(time_t lastSchematic, ta_uint32 maxSize = 10);
		std::vector<MmsRequestScheduledData*> getMmsCounterData(time_t latestCounter, ta_uint32 maxSize = 10);
		std::vector<MmsRequestScheduledData*> getMmsMeasurementData(time_t latestAlarm, ta_uint32 maxSize = 10);
        /**
         * getMmsRequestAlarmData
         *
         * get MMS request of alarm
		...
		bool deleteMmsRequestAlarmData(std::vector<std::string>& vec);
		bool deleteMmsRequestSchematicData(std::vector<std::string>& vec);
		bool deleteMmsRequestScheduledData(std::vector<std::string>& vec);
    ...
    private:
        ...
        MmsRequestAlarmData* createMmsRequestAlarmData(ta_uint32 row, IData* data);
        MmsRequestSchematicData* createMmsRequestSchematicData(ta_uint32 row, IData* data);
        MmsRequestScheduledData* createMmsRequestScheduledData(ta_uint32 row, IData* data);
		...
		std::string  getDeleteSQL(IDatabase* databaseConnection, std::vector<std::string>& keylist);
