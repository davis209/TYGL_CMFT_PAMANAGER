 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/NamedFilterAccessFactory.cpp $
...
namespace TA_Base_Core
{
    ...
    NamedFilterAccessFactory::~NamedFilterAccessFactory()
    {
        TA_THREADGUARD(m_staticFiltersLock);
    ...
    void NamedFilterAccessFactory::registerStaticNamedFilter(std::auto_ptr<NamedFilter> namedFilter)
    {
		...
		std::string filterName = namedFilter->getName();

        // Add the filter to the map.
        m_staticFilters[filterName] = namedFilter.release();
    }
    ...
    std::auto_ptr<NamedFilter> NamedFilterAccessFactory::getNamedFilter(const std::string& filterName)
    {
        ...
        do
        {
            ...
            for (ta_uint32 i = 0; i < data->getNumRows(); i++)
            {
                ...
                ta_uint32 listType = data->getUnsignedLongData(i, COLUMN_LIST_TYPE);
    ...
    ta_uint32 NamedFilterAccessFactory::getApplicationKey()
    {
