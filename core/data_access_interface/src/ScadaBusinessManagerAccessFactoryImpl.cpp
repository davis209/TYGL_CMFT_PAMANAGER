...
namespace TA_Base_Core
{
    ...
    ScadaBusinessItemVector* ScadaBusinessManagerAccessFactoryImpl::loadDataFromDb(const std::string& agentName)
    {
        ...
        try
        {
            databaseConnection = DatabaseFactory::getInstance().getDatabase(Scada_Cd, Read);
        }
        catch (const TA_Base_Core::DatabaseException& ex)
        {
            LOG_EXCEPTION("getDatabase DatabaseException", ex.what());
            FUNCTION_EXIT;
            return ivec;
        }
        ...
        {
            LOG_EXCEPTION("query DBException", ex.what());
        }
        catch (const TA_Base_Core::DatabaseException& ex)
        {
            LOG_EXCEPTION("query DatabaseException", ex.what());
        }
        catch (const TA_Base_Core::DataException& ex)
        {
            LOG_EXCEPTION("query DataException", ex.what());
        }
                                                              ...
                                                              IDatabase* databaseConnection, const std::string& agentName)
    {
        ...
        ta_uint32 interval = 0;
        std::string entityList = "";
        ScadaBusinessHeartbeat* sbitem = NULL;

        ta_uint32 size = data->getNumRows();
        LOG_DEBUG("Loaded %d heartbeat for %s", size, agentName);

        for (ta_uint32 i = 0; i < size; ++i)
        {
            ...
            LOG_DEBUG("Created heartbeat %d, interval=%d", i, interval);
        }
                                                                    ...
                                                                    IDatabase* databaseConnection, const std::string& agentName)
    {
        ...
        ta_uint32 size = data->getNumRows();
        LOG_DEBUG("Loaded %d monitor_and_write for %s", size, agentName);
        ...
        for (ta_uint32 i = 0; i < size; ++i)
        {
            ...
            LOG_DEBUG("Created monitor_and_write %d, fep=%s(%d)", i, fepSubsystem, fepLocation);
        }
