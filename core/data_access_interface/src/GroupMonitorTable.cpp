 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/GroupMonitorTable.cpp $
...
{
    GroupMonitorTable::Table& GroupMonitorTable::select()
    {
        static GroupMonitorTable::Table s_table;
        static std::once_flag s_once;
        std::call_once(s_once, [&]
        {
            auto* db = DatabaseFactory::getInstance().getDatabase(/*SysController_SCd*/SysController_Cd, Read);
            auto data = db->executeQueryAll("SELECT * FROM sn_group_monitor");
            ...
            for (auto row : *data)
            {
                ...
                {
                    row.getUnsignedLongData("SNSCON_ID"),
    ...
    GroupMonitorTable::Table GroupMonitorTable::select(Where where)
    {
        return stdutil::filter_copy(select(), where);
    }

    boost::optional<GroupMonitorTable::Row&> GroupMonitorTable::selectFirst(Where where)
    {
