 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/AlarmTypeHelper.cpp $
 * @author:  Nick Jardine
 * @version: $Revision: #3 $
 *
...
{
    const ta_uint32 AlarmTypeHelper::NEW_ALARM_TYPES_STARTING_PKEY = 55000;
    ...
    AlarmTypeHelper::AlarmTypeHelper(ta_uint32 key)
        :
        m_isValidData(false),
        m_key(key),
        m_name(""),
    ...
    AlarmTypeHelper::AlarmTypeHelper(const ta_uint32 row, TA_Base_Core::IData& data)
        :
        m_isDeleted(false), //TD17081 - must be initialized to false or assertions occur
        m_isValidData(false),
        m_MmsStateType(IAlarmData::TYPE_NONE) //TD17628
    {
        FUNCTION_ENTRY("AlarmTypeHelper(const ta_uint32 , TA_Base_Core::IData&)");
    ...
    ta_uint32 AlarmTypeHelper::getKey()
    {
    ...
    void AlarmTypeHelper::setDescription(const std::string& description)
    {
        ...
        FUNCTION_ENTRY("AlarmTypeHelper::setDescription");
    ...
    ta_uint32 AlarmTypeHelper::getSeverityKey()
    {
        TA_ASSERT(!m_isDeleted,
                  "Operation attempted after object deleteAlarmType called.");
    ...
    void AlarmTypeHelper::setSeverityKey(const ta_uint32 severityKey)
    {
        FUNCTION_ENTRY("AlarmTypeHelper::setSeverityKey");
        TA_ASSERT(!m_isDeleted,
                  "Operation attempted after object deleteAlarmType called.");
    ...
    ta_uint32 AlarmTypeHelper::getContextKey()
    {
        TA_ASSERT(!m_isDeleted,
                  "Operation attempted after object deleteAlarmType called.");
    ...
    {
        FUNCTION_ENTRY("AlarmTypeHelper::setPrintAlarmStatus");
        TA_ASSERT(!m_isDeleted,
                  "Operation attempted after object deleteAlarmType called.");
    ...
    {
        FUNCTION_ENTRY("AlarmTypeHelper::setPersistAlarmStatus");
        TA_ASSERT(!m_isDeleted,
                  "Operation attempted after object deleteAlarmType called.");
    ...
    {
        FUNCTION_ENTRY("AlarmTypeHelper::invalidate");
    ...
    void AlarmTypeHelper::createNewRowInDatabase()
    {
        ...
        This functionality is not currently required.
            ...
            IDatabase* databaseConnection =
                    DatabaseFactory::getInstance().getDatabase(Alarms_Cd, Read);
    ...
    {
        FUNCTION_ENTRY("AlarmTypeHelper::reload");
        TA_ASSERT(!m_isDeleted,
                  "Operation attempted after object deleteAlarmType called.");

        // get a connection to the database
        IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase(Alarms_SCd/*Alarms_Cd*/, Read);
    ...
    void AlarmTypeHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data)
    {
    ...
    {
        FUNCTION_ENTRY("AlarmTypeHelper::writeAlarmTypeData");
        TA_ASSERT(!m_isDeleted,
                  "Operation attempted after object deleteAlarmType called.");
        ...
        if (m_name.empty())
        {
            ...
            TA_THROW(TA_Base_Core::DataConfigurationException("Alarm type data not fully specified. Alarm type cannot be written to database", fieldNames));
        }

        // Write the Alarm Type to the database

        // get a connection to the database
        IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase(Alarms_SCd/*Alarms_Cd*/, Write);
    ...
    {
        FUNCTION_ENTRY("AlarmTypeHelper::deleteAlarmType");
        TA_ASSERT(!m_isDeleted,
                  "Operation attempted after object deleteAlarmType called.");
        ...
        // get a connection to the database
        IDatabase* databaseConnection =
            DatabaseFactory::getInstance().getDatabase(Alarms_SCd/*Alarms_Cd*/, Write);
    ...
    {
        ta_uint32 entitykey = 0;
        ta_uint32 entitytypekey = 0;
