 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/MessageData.cpp $
 * @author:  Nick Jardine
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Core
{
    ...
    void MessageData::setType(const ta_uint32 typeKey)
    {
        if (!m_isValidData && std::string("") != m_key)
        {
            reload();
    ...
    ta_uint32 MessageData::getEntityKey()
    {
        if (m_isValidData || std::string("") == m_key)
        {
    ...
    void MessageData::setEntityKey(const ta_uint32 key)
    {
        if (!m_isValidData && std::string("") != m_key)
        {
            reload();
    ...
    // TD13673
    ta_uint32 MessageData::getLocationKey()
    {
        if (m_isValidData || "" == m_key)
        {
            TA_ASSERT(TA_MAX_UINT32 != m_locationKey, "The location ID must be set before it can be retrieved");
        }
        else
        {
            reload();
    ...
    void MessageData::setLocationKey(ta_uint32 locationKey)
    {
        if (!m_isValidData && TA_MAX_UINT32 != m_locationKey)
        {
            reload();
        }
    ...
    {
        FUNCTION_ENTRY("MessageData::invalidate");
    ...
    {
        FUNCTION_ENTRY("MessageData::reload");
    ...
    {
        FUNCTION_ENTRY("MessageData::applyChanges");
        ...
        bool createTimeValid = TA_MAX_UINT32 != m_createTime.time;
        bool logTimeValid = TA_MAX_UINT32 != m_logTime.time;
    ...
    // CL-21508 end
    {
        ...
        char logTimeString[24] = {0};   // buffer [22] not ta_int32 enough to contain "%02d/%02d/%04d %02d:%02d:%02d.%03d"
    ...
    {
        LOG_DEBUG("MessageData::updateAlarmExistingMessage(),it should not be excuted.");
    }
    void MessageData::updateOthersExistingMessage()
    {
        FUNCTION_ENTRY("MessageData::updateExistingMessage");
    ...
    void MessageData::setEventLelOrSeverityKey(const ta_uint32 eventLelOrSeverityKey)
    {
        m_eventLelOrSeverityKey = eventLelOrSeverityKey;
    }

    void MessageData::setAlarmTypeKey(const ta_uint32 alarmTypeKey)
    {
    ...
    {
        FUNCTION_ENTRY("MessageData::createNewMessageWithAlarm");
