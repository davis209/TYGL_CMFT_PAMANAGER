 ...
 * in any form.
 *
 * Source: $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/AlarmSeverityHelper.cpp $
...
{
    AlarmSeverityHelper::AlarmSeverityHelper(const ta_uint32 key)
        :
        m_name(""),
        m_key(key),
        m_dateCreated(0),
    ...
    AlarmSeverityHelper::AlarmSeverityHelper(const ta_uint32 row, TA_Base_Core::IData& data)
        :
        m_isValidData(false)
    {
        FUNCTION_ENTRY("AlarmSeverityHelper(const ta_uint32 , TA_Base_Core::IData&)");
    ...
    void AlarmSeverityHelper::setSeverityColourKey(ta_uint32 colour, IAlarmSeverityData::EColourType type)
    {
        if (! m_isValidData)
        {
            reload();
        ...
        std::map<IAlarmSeverityData::EColourType, ta_uint32>::iterator matching;
        ...
        {
            m_severityColourKey.insert(std::map<IAlarmSeverityData::EColourType, ta_uint32>::value_type(type, colour));
        }
    }

    ta_uint32 AlarmSeverityHelper::getKey()
    {
        return m_key;
    }
    ...
    ta_uint32 AlarmSeverityHelper::getSeverityColourKey(IAlarmSeverityData::EColourType type)
    {
        if (! m_isValidData)
        {
            reload();
    ...
    {
        FUNCTION_ENTRY("AlarmSeverityHelper::invalidate");
    ...
    {
        FUNCTION_ENTRY("AlarmSeverityHelper::reload");
        ...
        if (NULL == data.get() || 0 == data->getNumRows()) // No entry found with the specified pkey
        {
            ...
            sprintf(reasonMessage, "No severity found for pkey = %u", m_key);
    ...
    void AlarmSeverityHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data)
    {
        ...
        */
        ...
        m_severityColourKey.insert(std::map<IAlarmSeverityData::EColourType, ta_uint32>::value_type(
    ...
    void AlarmSeverityHelper::applyChanges()
    {
        ...
        IDatabase* databaseConnection =
            DatabaseFactory::getInstance().getDatabase(Alarms_SCd/*Alarms_Cd*/, Read);
