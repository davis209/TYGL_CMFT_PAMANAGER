 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/ActionGroupHelper.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #4 $
 *
...
namespace TA_Base_Core
{
    ...
    ActionGroupHelper::ActionGroupHelper(const ta_uint32 key, const std::string& name)
        : m_key(key),
          m_name(name),
          m_isControlType(false),
          m_dateCreated(0),
    ...
    ActionGroupHelper::ActionGroupHelper(const ta_uint32 row, TA_Base_Core::IData& data, std::vector<ta_uint32>& actions)
        :   m_isNew(false),
            m_isValidData(false)
    {
        FUNCTION_ENTRY("ActionGroupHelper(const ta_uint32 , TA_Base_Core::IData&)");
    ...
    ta_uint32 ActionGroupHelper::getKey()
    {
    ...
    bool ActionGroupHelper::isActionIncluded(ta_uint32 actionKey)
    {
    ...
    void ActionGroupHelper::addAction(ta_uint32 actionKey)
    {
    ...
    void ActionGroupHelper::removeAction(ta_uint32 actionKey)
    {
    ...
    void ActionGroupHelper::reload()
    {
        ...
        do
        {
            for (ta_uint32 i = 0; i < data2->getNumRows(); i++)
            {
                try
                {
                    ta_uint32 actionId = data2->getUnsignedLongData(i, columnNames[0]);
    ...
    void ActionGroupHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data, std::vector<ta_uint32>& actions)
    {
    ...
    void ActionGroupHelper::modifyExisting()
    {
        ...
        LOG_DEBUG("ActionGroup %s already exists. It's details will be updated.", m_name);
    ...
    void ActionGroupHelper::addNew()
    {
        ...
        LOG_DEBUG("ActionGroup %s is new. It will be added to the database.", m_name);
    ...
    void ActionGroupHelper::updateActions()
    {
        ...
        std::set<ta_uint32> localActionsToAdd = m_actions;
        std::vector<ta_uint32> databaseActionsToRemove;
        ...
        do
        {
            ...
            {
                ta_uint32 currentKey = data->getUnsignedLongData(i, actionKeyColumn);
        ...
        if (!databaseActionsToRemove.empty())
        {
            ...
            for (std::vector<ta_uint32>::iterator iterToRemove = databaseActionsToRemove.begin();
        ...
        for (std::set<ta_uint32>::iterator iterToAdd = localActionsToAdd.begin();
    ...
    void ActionGroupHelper::checkForLinkedProfiles()
    {
        ...
        const ta_uint32 CHILD_ENTITIES = 10;
        // get a connection to the database
        ...
        if (data->getNumRows() > 0)
        {
            ...
            TA_THROW(DataException("This ActionGroup cannot be deleted as it has profiles using it.", DataException::CANNOT_DELETE, linkedProfiles));
        }
