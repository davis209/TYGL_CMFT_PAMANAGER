 ...
 * in any form.
 *
 * Source:   $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/ActionHelper.cpp $
 * @author:  Ripple
 * @version: $Revision: #4 $
 *
...
{
    ActionHelper::ActionHelper(ta_uint32 key)
        :
        m_isValidData(false),
        m_key(key),
        m_name(""),
    ...
    ActionHelper::ActionHelper(const ta_uint32 key, const std::string& name, const std::string& description)
        :
        m_isValidData(false),
        m_key(key),
        m_name(name),
    ...
    ActionHelper::ActionHelper(const ta_uint32 key, const std::string& name, const std::string& description, const bool isDisplayOnly)
        :
        m_isValidData(false),
        m_key(key),
        m_name(name),
    ...
    ActionHelper::ActionHelper(const ta_uint32 row, TA_Base_Core::IData& data, std::vector<std::string>& actionGroups)
        :
        m_isValidData(false)
    {
        FUNCTION_ENTRY("ActionHelper(const ta_uint32 , TA_Base_Core::IData&)");

        reloadUsing(row, data, actionGroups);
        FUNCTION_EXIT;
    }

    ta_uint32 ActionHelper::getKey()
    {
        return m_key;
    }
    ...
    void ActionHelper::reload()
    {
        ...
        TA_Base_Core::IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase({ OfflineSecurity_Cd, OfflineSecurity_Cached}, Read);
    ...
    void ActionHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data, std::vector<std::string>& actionGroups)
    {
