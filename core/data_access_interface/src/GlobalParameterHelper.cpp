 ...
 * in any form.
 *
 * Source: $File: //depot/TYGL_CMFT_TIP/TA_BASE/transactive/core/data_access_interface/src/GlobalParameterHelper.cpp $
 * @author Karen Graham
 * @version $Revision: #4 $
...
{
    static const ta_uint32 INVALID_KEY = -1;
    ...
    GlobalParameterHelper::GlobalParameterHelper(const ta_uint32 key)
        : m_key(key),
          m_name(""),
          m_value(""),
          m_subsystemKey(INVALID_KEY),
    ...
    GlobalParameterHelper::GlobalParameterHelper(const ta_uint32 row, TA_Base_Core::IData& data)
        :   m_isNew(false),
            m_isValidData(false)
    {
        FUNCTION_ENTRY("GlobalParameterHelper(const ta_uint32 , TA_Base_Core::IData&)");
    ...
    ta_uint32 GlobalParameterHelper::getKey()
    {
    ...
    ta_uint32 GlobalParameterHelper::getSubsystemKey()
    {
        // We don't want to call reload() if this is a new GlobalParameter and it has not been written to the database
        if (!m_isValidData && !m_isNew)
        {
    ...
    void GlobalParameterHelper::setSubsystemKey(ta_uint32 subsystemKey)
    {
        if (!m_isValidData && !m_isNew)
        {
            reload();
    ...
    void GlobalParameterHelper::reloadUsing(const ta_uint32 row, TA_Base_Core::IData& data)
    {
    ...
    void GlobalParameterHelper::addNewGlobalParameter()
    {
        ...
        LOG_DEBUG("GlobalParameter %s is a new GlobalParameter. It will be added to the database.", m_name);
    ...
    void GlobalParameterHelper::modifyExistingGlobalParameter()
    {
        ...
        LOG_DEBUG("GlobalParameter %s already exists. It's details will be updated.", m_name);
    ...
    //throw(TA_Base_Core::TransactiveException)
    {
        ...
        if (m_isNew)
        {
            IDatabase* pDatabaseConn = DatabaseFactory::getInstance().getDatabase(GlobalParameter_Cd/*GlobalParameter_Cd*/, Read);
            /*std::ostringstream sql;
    ...
    //throw(TA_Base_Core::TransactiveException)
    {
        ...
        //
        ////////////////////////////////////////////////////////////////////////////
        IDatabase* databaseConnection = DatabaseFactory::getInstance().getDatabase(GlobalParameter_Cd/*GlobalParameter_Cd*/, Read);
        /*std::ostringstream sql;
        sql << "select GPARAM_ID from GLOBAL_PARAMETER where UPPER(NAME) = '"
            << databaseConnection->escapeInsertString(boost::to_upper_copy(m_name))
        ...
        databaseConnection->prepareSQLStatement(strSql, GLOBAL_PARAMETER_SELECT_16002,
                                                databaseConnection->escapeInsertString(boost::to_upper_copy(m_name)), m_subsystemKey, m_key);
    ...
    std::string GlobalParameterHelper::getLocalDatabaseName()
    {
        if (m_localDatabase.empty())
        {
            ...
            try
            {
                ...
                m_localDatabase = DatabaseFactory::getInstance().getDatabaseName(strDbConnFile, GlobalParameter_Cd, Write);
