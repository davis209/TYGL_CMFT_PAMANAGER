...
namespace TA_IRS_App
{
    ...
    namespace
    {
        std::string getStatusName(const TA_IRS_App::ExecutionStatus status)
        {
            switch (status)
            {
            case TA_IRS_App::ExecutionStatus::Success:       return "SUCCESS";
            case TA_IRS_App::ExecutionStatus::PartialFailed: return "PARTIAL_FAILED";
            case TA_IRS_App::ExecutionStatus::Cancelled:     return "CANCELLED";
    ...
    std::vector<TA_IRS_App::BroadcastSchedule> PaBroadcastScheduleAccessFactory::getDuePaBroadcastSchedules(
        ...
        const std::function<std::string(const TA_IRS_App::BroadcastSchedule&)>& calculateNextRun)
    {
        ...
        try
        {
            ...
            databaseConnection->prepareSQLStatement(selectSql, PABROADCASTSCHEDULE_SELECT_90001, 
				locationKey, maximumCount);
            std::auto_ptr<TA_Base_Core::IData> data(databaseConnection->executeQuery(selectSql, columnNames));
            std::vector<TA_IRS_App::BroadcastSchedule> schedules;
            for (int row = 0; row < data->getNumRows(); ++row)
            {
                TA_IRS_App::BroadcastSchedule schedule;
            ...
            for (std::vector<TA_IRS_App::BroadcastSchedule>::iterator schedule = schedules.begin(); schedule != schedules.end(); ++schedule)
            {
