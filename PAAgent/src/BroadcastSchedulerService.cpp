...
void BroadcastSchedulerService::Start() {
    ...
    executor_->Start();
	LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Scheduler worker started");
...
void BroadcastSchedulerService::Stop() {
    ...
    executor_->Stop();
    LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"Scheduler worker stopping");
...
void BroadcastSchedulerService::WorkerLoop() {
    while (!stopRequested_.load()) {
        try {
            ...
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError,"Schedule polling failed: " + std::string(error.what()));
        } catch (...) {
            LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError,"Schedule polling failed: unknown error");
        }
...
void BroadcastSchedulerService::DispatchDueSchedules() {
    ...
    const auto due = TA_IRS_App::PaBroadcastScheduleAccessFactory::getInstance().getDuePaBroadcastSchedules(
        config_.locationId, config_.maxDueSchedulesPerPoll,
        [now](const BroadcastSchedule& schedule) { return ComputeNextRun(schedule, now); });
    if (!due.empty())
        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"Found " + std::to_string(due.size()) + " due schedule(s) for location " + std::to_string(config_.locationId));
...
void BroadcastSchedulerService::RunSchedule(BroadcastSchedule schedule) {
    LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,"Executing schedule " + std::to_string(schedule.scheduleId) + " (" + schedule.scheduleName + "), run " + std::to_string(schedule.runId));
    ...
    if (execution.status == ExecutionStatus::Success) LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo,completion);
    else LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError,completion + (execution.errorMessage.empty() ? std::string() : ": " + execution.errorMessage));

    try {
        TA_IRS_App::PaBroadcastScheduleAccessFactory::getInstance().updatePaBroadcastScheduleLog(schedule.runId, execution);
    } catch (const std::exception& error) {
        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError,"Unable to update execution log for schedule " + std::to_string(schedule.scheduleId) + ": " + error.what());
    } catch (...) {
        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError,"Unable to update execution log for schedule " + std::to_string(schedule.scheduleId) + ": unknown error");
    }
}
