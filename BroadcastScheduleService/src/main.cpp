...
void PrintUsage() {
    std::cout << "Usage: BroadcastScheduleService --location-id <id> --pa-host <ip-or-hostname> --pa-port <port> "
                 "[--poll-seconds <seconds>] [--pa-console-id <id>]\n";
}
...
int main(int argc, char* argv[]) {
    try {
        ...
        if (config.locationId <= 0 || paDevice.host.empty() || paDevice.port == 0) {
            PrintUsage();
            return 2;
        }
